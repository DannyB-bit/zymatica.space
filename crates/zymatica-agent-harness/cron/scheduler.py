...
Uses a file-based lock (~/.zymatica/cron/.tick.lock) so only one tick
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    from zymatica_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from zymatica_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
# (ZYMATICA_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse ZYMATICA_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("ZYMATICA_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid ZYMATICA_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.zymatica/cron/usage_audit.jsonl.
...
def _get_hermes_home() -> Path:
    ...
    profile-scoped gateway, so resolving the active ZYMATICA_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ZYMATICA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ZYMATICA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within ZYMATICA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ZYMATICA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within ZYMATICA_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("ZYMATICA_MODEL") or ""

    from zymatica_cli.auth import AuthError

    try:
        from zymatica_cli.runtime_provider import resolve_runtime_provider
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from ZYMATICA_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("ZYMATICA_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ZYMATICA_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    # gateway message. Do not seed ZYMATICA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ZYMATICA_SESSION_PLATFORM
    #     and ZYMATICA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ZYMATICA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ZYMATICA_SESSION_PLATFORM == "telegram".
    ...
    #     ZYMATICA_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ZYMATICA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ZYMATICA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ZYMATICA_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # ZYMATICA_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ ZYMATICA_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "ZYMATICA_CRON_AUTO_DELIVER_PLATFORM",
        "ZYMATICA_CRON_AUTO_DELIVER_CHAT_ID",
        "ZYMATICA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["ZYMATICA_CRON_SESSION"]
    ...
    try:
        ...
        # toolset includes `cronjob`, running with ZYMATICA_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $ZYMATICA_KANBAN_TASK -- letting
        ...
        # recorded this ZYMATICA_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        from zymatica_cli.env_loader import (
            load_hermes_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["ZYMATICA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ZYMATICA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ZYMATICA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > ZYMATICA_MODEL env > config.yaml ``model:``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"ZYMATICA_MODEL={os.getenv('ZYMATICA_MODEL', '')!r}, "
        ...
        try:
            from zymatica_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("ZYMATICA_PREFILL_MESSAGES_FILE", "")
        ...
        from zymatica_cli.runtime_provider import (
        ...
        try:
            # Do not inject ZYMATICA_INFERENCE_PROVIDER here. resolve_runtime_provider()
            ...
            for entry in fb_list:
                ...
                if not fb_provider or not fb_model:
                    continue
                try:
                    from zymatica_cli.fallback_config import resolve_entry_api_key
        ...
        agent = AIAgent(
            ...
            # ZYMATICA_HOME. When a workdir is configured, also inject project
        ...
        # override via ZYMATICA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from zymatica_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Set ZYMATICA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ZYMATICA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ZYMATICA_CRON_MAX_PARALLEL value; defaulting to unbounded")
