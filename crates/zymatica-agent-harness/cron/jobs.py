...
Jobs are stored in ~/.zymatica/cron/jobs.json
Output is saved to ~/.zymatica/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own ZYMATICA_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same ZYMATICA_HOME — so a job authored in profile
# `coder` lives in `~/.zymatica/profiles/coder/cron/jobs.json` and executes with
...
# into one shared `jobs.json` and run them under whatever ZYMATICA_HOME the
...
ZYMATICA_DIR = get_hermes_home().resolve()
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_hermes_home()
       (context-local override, then the ZYMATICA_HOME env var) — so a test
       or embedder that re-points ZYMATICA_HOME after this module was
...
# the cron inactivity timeout is disabled (ZYMATICA_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. ZYMATICA_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``ZYMATICA_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall zymatica or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    """
    try:
        from zymatica_cli.config import _expand_env_vars, read_user_config_raw
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.zymatica/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.zymatica/scripts/, .sh/.bash via bash, else Python). Each
...
def _machine_id() -> str:
    ...
    Uses ``ZYMATICA_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("ZYMATICA_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from ZYMATICA_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one ZYMATICA_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from ZYMATICA_CRON_TIMEOUT),
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``ZYMATICA_HOME/skills`` or an
