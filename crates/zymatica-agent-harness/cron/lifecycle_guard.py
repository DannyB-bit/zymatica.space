...
``hermes gateway restart`` (or ``launchctl kickstart ai.zymatica.gateway``
...
commands and shell scripts they reference when ``_ZYMATICA_GATEWAY=1``. It also
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    ...
    # labels look like `ai.zymatica.gateway` / `hermes-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated hermes services (e.g.
    # `launchctl unload ai.zymatica.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.zymatica.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.zymatica.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.zymatica.svc-reload-tmp``)
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".zymatica"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<ZYMATICA_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.zymatica/scripts/restart.sh``) but is passed as the bare name
    ...
    try:
        ...
        # neither ZYMATICA_HOME nor HOME is resolvable (launchd/systemd
