...
def _build_child_agent(
    ...
    try:
        from zymatica_cli.lifecycle import invoke_hook as _invoke_hook
        _invoke_hook(
            "subagent_start",
            parent_session_id=getattr(parent_agent, "session_id", None),
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.zymatica/logs/subagent-<sid>-<ts>.log``
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("ZYMATICA_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from zymatica_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # ZYMATICA_SESSION_ID ContextVar and os.environ with the subagent's internal
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("ZYMATICA_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # ZYMATICA_SESSION_ID here would return the subagent's internal id.
        ...
        try:
            ...
            _source = get_session_env("ZYMATICA_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("ZYMATICA_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ZYMATICA_SESSION_KEY is not in the CLI
...
def _load_config() -> dict:
    ...
    ZYMATICA_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ZYMATICA_IGNORE_USER_CONFIG=1`` (``hermes chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ZYMATICA_IGNORE_USER_CONFIG") == "1"
