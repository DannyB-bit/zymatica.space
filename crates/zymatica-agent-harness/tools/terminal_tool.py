...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ZYMATICA_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ZYMATICA_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ZYMATICA_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ZYMATICA_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ZYMATICA_INTERACTIVE=1 or a registered sudo password callback):
    ...
    should_prompt_for_sudo = (
        env_var_enabled("ZYMATICA_INTERACTIVE") or has_sudo_prompt_callback
    )
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``zymatica=1`` for the current
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.zymatica/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    try:
        from zymatica_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from zymatica_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    try:
        ...
        if os.environ.get("_ZYMATICA_GATEWAY") == "1":
        ...
        if background:
            ...
            try:
                ...
                # zymatica dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in zymatica dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("ZYMATICA_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("ZYMATICA_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("ZYMATICA_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("ZYMATICA_SESSION_USER_ID", "")
                            _gw_user_name = _gse("ZYMATICA_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("ZYMATICA_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("ZYMATICA_INTERACTIVE"):
                    output += (
