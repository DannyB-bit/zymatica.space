...
# All skills live in ~/.zymatica/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
ZYMATICA_HOME = get_hermes_home()
SKILLS_DIR = ZYMATICA_HOME / "skills"
...
def _skills_dir() -> Path:
    ...
    set ZYMATICA_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped ZYMATICA_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ZYMATICA_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ZYMATICA_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ZYMATICA_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ZYMATICA_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ZYMATICA_SESSION_PLATFORM"))
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.zymatica/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ZYMATICA_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ZYMATICA_PLATFORM`` environment variable
    3. ``ZYMATICA_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from zymatica_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ZYMATICA_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.zymatica/skills/ and external dirs.
    ...
    # (_skills_dir() resolves the LIVE profile ZYMATICA_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from zymatica_cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.zymatica/skills/): {skill_md}")
        ...
        # Check metadata.zymatica.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
