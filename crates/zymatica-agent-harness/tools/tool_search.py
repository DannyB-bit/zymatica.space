...
* Core tools defined in ``toolsets._ZYMATICA_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    """
    try:
        from toolsets import _ZYMATICA_CORE_TOOLS
        return frozenset(_ZYMATICA_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_ZYMATICA_CORE_TOOLS``. Core tools are never deferred
