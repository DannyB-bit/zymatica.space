...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``zymatica_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from zymatica_constants import get_subprocess_home
...
def _get_hermes_config_resolved() -> str | None:
    ...
    try:
        ...
        try:
            _hermes_config_resolved = str(Path(_expand_tilde("~/.zymatica/config.yaml")).resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            ...
            "Edit ~/.zymatica/config.yaml directly or use 'hermes config' instead."
        )
    return None
...
# .cursorrules (or a project-local .zymatica config tree) outlives the current
...
def _get_real_hermes_home() -> str | None:
    ...
    try:
        ...
        try:
            _real_hermes_home_cached = os.path.realpath(_expand_tilde("~/.zymatica"))
        except Exception:
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from zymatica_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.zymatica home is governed by its own guards
    ...
    # ``.zymatica`` component rule below, which would otherwise match the
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .zymatica config dirs (e.g. <repo>/.zymatica/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.zymatica`` — matching
        # any ancestor named .zymatica would gate every write inside a
        # checkout that happens to live under ~/.zymatica (e.g. the
        # zymatica repo itself at ~/.zymatica/zymatica).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".zymatica":
            return candidate
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.zymatica/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.zymatica/…`` path.
    ...
    # in a session that cd'd into ``~/.zymatica/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # and block credential stores under ZYMATICA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ZYMATICA_HOME (e.g. "auth.json") still hits the
