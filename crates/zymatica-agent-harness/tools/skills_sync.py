...
Copies bundled skills from the repo's skills/ directory into ~/.zymatica/skills/
...
The manifest lives at ~/.zymatica/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from zymatica_constants import get_bundled_skills_dir, get_hermes_home, get_optional_skills_dir
...
# and by the installer's `--no-skills` flag (the default ~/.zymatica profile).
# When present in ZYMATICA_HOME, sync_skills() is a no-op so neither the
...
def _get_bundled_dir() -> Path:
    ...
    Checks ZYMATICA_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.zymatica/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.zymatica/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.zymatica/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.zymatica) that wrote the
    ...
    if (ZYMATICA_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.zymatica/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ZYMATICA_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.zymatica/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``ZYMATICA_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ZYMATICA_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ZYMATICA_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            ZYMATICA_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ZYMATICA_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.zymatica/skills/ ...")
