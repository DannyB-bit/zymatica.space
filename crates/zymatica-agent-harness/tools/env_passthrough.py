...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    token, or similar) per ``_ZYMATICA_PROVIDER_ENV_BLOCKLIST``.
    ...
    if _is_hermes_internal_secret(name):
        return True
    return name in _ZYMATICA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_ZYMATICA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                ...
                "credential %r (blocked by _ZYMATICA_PROVIDER_ENV_BLOCKLIST). "
            ...
            for item in passthrough:
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        ...
                        "by _ZYMATICA_PROVIDER_ENV_BLOCKLIST). Operator "
