...
from zymatica_cli.config import cfg_get
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ZYMATICA_YOLO_MODE", ""))
...
# os.environ["ZYMATICA_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ZYMATICA_INTERACTIVE.
...
def set_hermes_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["ZYMATICA_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ZYMATICA_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ZYMATICA_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hermes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ZYMATICA_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ZYMATICA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ZYMATICA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ZYMATICA_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ZYMATICA_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("ZYMATICA_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ZYMATICA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ZYMATICA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ZYMATICA_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("ZYMATICA_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ZYMATICA_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.zymatica/config.yaml. The
# resolved-absolute form is folded into the ~/.zymatica/ patterns at detection
...
# go stale when ZYMATICA_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ZYMATICA_ENV_PATH = (
    r'(?:~\/\.zymatica/|'
    r'(?:\$home|\$\{home\})/\.zymatica/|'
    r'(?:\$hermes_home|\$\{hermes_home\})/)'
    r'\.env\b'
)
# ~/.zymatica/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ZYMATICA_ENV_PATH; matches the ZYMATICA_HOME override form as
# well as ~/.zymatica/.
_ZYMATICA_CONFIG_PATH = (
    r'(?:~\/\.zymatica/|'
    r'(?:\$home|\$\{home\})/\.zymatica/|'
    r'(?:\$hermes_home|\$\{hermes_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_ZYMATICA_ENV_PATH}|'
    rf'{_ZYMATICA_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # directly against the service label (commonly `ai.zymatica.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hermes|ai\.zymatica)\b', "stop/restart hermes launchd service (kills running agents)"),
    ...
    # ~/.zymatica/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hermes-managed security file (~/.zymatica/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ZYMATICA_CONFIG_PATH}|{_ZYMATICA_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ZYMATICA_CONFIG_PATH}|{_ZYMATICA_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ZYMATICA_CONFIG_PATH}|{_ZYMATICA_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.zymatica/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ZYMATICA_HOME even when those are set after this module is
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ZYMATICA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.zymatica``); the matched
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.zymatica/``.

    Resolves the active ``ZYMATICA_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.zymatica/`` so the static ``_ZYMATICA_CONFIG_PATH`` / ``_ZYMATICA_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hermes/.zymatica/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ZYMATICA_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.zymatica")
...
def load_permanent_allowlist() -> set:
    ...
    """
    try:
        from zymatica_cli.config import load_config_readonly
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["ZYMATICA_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ZYMATICA_SPINNER_PAUSE" in os.environ:
            del os.environ["ZYMATICA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``ZYMATICA_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ZYMATICA_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set ZYMATICA_INTERACTIVE or "
                "ZYMATICA_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ZYMATICA_INTERACTIVE or "
            "ZYMATICA_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ZYMATICA_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ZYMATICA_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from zymatica_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from zymatica_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ZYMATICA_EXEC_ASK")
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from zymatica_cli.config import load_config_readonly as _load_cfg
