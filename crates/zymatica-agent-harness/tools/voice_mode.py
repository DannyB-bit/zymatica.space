...
def _voice_capture_install_hint() -> str:
    ...
    # ~/.zymatica/profiles/<name>/zymatica/venv/), `pip install` on the
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.zymatica/config.yaml during a
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("ZYMATICA_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    ZYMATICA_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
