...
def _global_allow_private_urls() -> bool:
    ...
    1. ``ZYMATICA_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("ZYMATICA_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``ZYMATICA_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
