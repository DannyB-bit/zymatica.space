...
# match a safe prefix, the operational ZYMATICA_ allowlist, or (on Windows) an
...
# ZYMATICA_HOME.
#
# NB: the broad "ZYMATICA_" prefix was deliberately removed (#27303) — it leaked
# ZYMATICA_*-named config that lacks a secret substring (e.g. ZYMATICA_BASE_URL,
# ZYMATICA_KANBAN_DB, ZYMATICA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ZYMATICA_CHILD_ALLOWED below; ZYMATICA_RPC_SOCKET /
# ZYMATICA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ZYMATICA_* vars the child legitimately needs by exact name — these
...
_ZYMATICA_CHILD_ALLOWED = frozenset({
    "ZYMATICA_HOME",
    "ZYMATICA_PROFILE",
    "ZYMATICA_CONFIG",
    "ZYMATICA_ENV",
    "ZYMATICA_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ZYMATICA_* vars (_ZYMATICA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ZYMATICA_* vars dropped by the tightened allowlist (#27303). The
    # broad "ZYMATICA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ZYMATICA_KANBAN_DB / ZYMATICA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ZYMATICA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ZYMATICA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ZYMATICA_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ZYMATICA_* var(s) from the "
...
def _connect():
    ...
    ZYMATICA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ZYMATICA_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ZYMATICA_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
_RPC_DIR = os.environ.get("ZYMATICA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"ZYMATICA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ZYMATICA_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ZYMATICA_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from ZYMATICA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  ZYMATICA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ZYMATICA_RPC_SOCKET"] = rpc_endpoint
        child_env["ZYMATICA_RPC_TOKEN"] = rpc_token
        ...
        # ZYMATICA_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ZYMATICA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ZYMATICA_TIMEZONE", None)
        ...
        if _uses_hermes_python_environment(_child_python):
            ...
            # per interpreter path) so "import zymatica_constants suddenly
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.zymatica/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (zymatica's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and zymatica's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.zymatica/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
