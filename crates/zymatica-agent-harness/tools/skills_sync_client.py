...
user-authored skills under ``~/.zymatica/skills/`` are eligible; bundled and
...
# namespace from zymatica's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ZYMATICA_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ZYMATICA_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ZYMATICA_SYNC_BASE_URL")
...
# precedence as base_url: the ZYMATICA_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   ZYMATICA_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ZYMATICA_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ZYMATICA_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``ZYMATICA_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    ...
    """
    return _sync_config_bool("ZYMATICA_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ZYMATICA_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.
    ...
    return _sync_config_bool(
        "ZYMATICA_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ZYMATICA_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.
    ...
    """
    return _sync_config_bool("ZYMATICA_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.zymatica/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.zymatica/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Hermes Cloud "on by default"):** every *eligible* skill syncs
      ...
      deployment sets (via ``ZYMATICA_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.zymatica/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.zymatica/skills/.sync_device_id.
    ...
    # declaratively via ZYMATICA_SYNC_DEVICE_NAME, so a hosted instance shows a
    ...
    env_name = (os.environ.get("ZYMATICA_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.zymatica/skills/.sync_device_id, overwriting
...
# ~/.zymatica/skills/.sync_state as JSON.
#
...
# path under ~/.zymatica/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.zymatica/skills/ (posix), or None."""
    try:
...
def pull_skills(
    ...
    *,
    ...
    ~/.zymatica/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "hermes skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ZYMATICA_SYNC_ENABLED)
...
#
# Org skills live under a DISTINCT local namespace, ~/.zymatica/skills/_org/
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.zymatica/skills/_org/<org_id>/``.
