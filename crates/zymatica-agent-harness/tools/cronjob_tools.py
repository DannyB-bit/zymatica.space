...
# uses 30s) — comfortably below the 1800s default ZYMATICA_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (ZYMATICA_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with ZYMATICA_CRON_TIMEOUT=0
...
#      saying "cat ~/.zymatica/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `zymatica-dev` skill contains a security postmortem mentioning
#      `cat ~/.zymatica/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("ZYMATICA_SESSION_PLATFORM")
    origin_chat_id = get_session_env("ZYMATICA_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("ZYMATICA_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes ZYMATICA_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``ZYMATICA_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``ZYMATICA_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("ZYMATICA_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("ZYMATICA_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("ZYMATICA_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("ZYMATICA_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ZYMATICA_HOME/scripts/.
    ...
    # Only relative paths within ~/.zymatica/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.zymatica/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.zymatica/scripts/ and use just the filename."
        )
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.zymatica/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("ZYMATICA_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("ZYMATICA_INTERACTIVE")
        or env_var_enabled("ZYMATICA_GATEWAY_SESSION")
        or env_var_enabled("ZYMATICA_EXEC_ASK")
    )
