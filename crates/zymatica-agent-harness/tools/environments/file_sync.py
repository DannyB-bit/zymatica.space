...
_FORCE_SYNC_ENV = "ZYMATICA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.zymatica") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.zymatica to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.zymatica", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ZYMATICA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.zymatica/`` directory as a tar archive,
    ...
    def _sync_back_transaction(self, hermes_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .zymatica/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.zymatica/skills/a.md`` →
        ``~/.zymatica/skills/a.md``, a new remote file at
        ``/root/.zymatica/skills/b.md`` maps to ``~/.zymatica/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
