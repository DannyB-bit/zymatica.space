...
# ZYMATICA_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ZYMATICA_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ZYMATICA_HOME}/sandboxes/.
    """
...
def _cwd_marker(session_id: str) -> str:
    return f"__ZYMATICA_CWD_{session_id}__"
...
# the FIRST session's ZYMATICA_SESSION_ID into the snapshot makes every LATER
...
# ZYMATICA_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# with one of these prefixes (or is ZYMATICA_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ZYMATICA_SESSION_|ZYMATICA_UI_SESSION_ID|ZYMATICA_CRON_AUTO_DELIVER_|ZYMATICA_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ZYMATICA_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ZYMATICA_SESSION_*} ${!ZYMATICA_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / ZYMATICA_AGENT are per-command attribution markers
        ...
        "AI_AGENT ZYMATICA_AGENT "
        f"ZYMATICA_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_ZYMATICA_RUNTIME_PASSTHROUGH_{name}"
        ...
        # ``ZYMATICA_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``zymatica`` — see huggingface.js
        ...
        parts.append(
            'export AI_AGENT="${AI_AGENT:-zymatica}" '
            'ZYMATICA_AGENT="${ZYMATICA_AGENT:-true}"'
        )
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ZYMATICA_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via ZYMATICA_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ZYMATICA_CWD_{session}__ marker from stdout output.
