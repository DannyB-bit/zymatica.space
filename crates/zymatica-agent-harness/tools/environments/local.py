...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Hermes was already launched from (for example ``zymatica``
    while the process cwd is ``~/.zymatica/zymatica``), passing it through
    unchanged makes the wrapper run ``cd zymatica`` *inside* the project
    ...
    # Common recovery for config values like ``zymatica`` when Hermes was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./zymatica``; use the current directory instead.
...
_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX = "_ZYMATICA_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from zymatica_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "ZYMATICA_DASHBOARD_SESSION_TOKEN",
...
_ZYMATICA_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _is_hermes_internal_secret(key: str) -> bool:
    ...
    ``_ZYMATICA_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
...
def _inject_context_hermes_home(env: dict) -> None:
    ...
    try:
        from zymatica_constants import get_hermes_home_override

        value = get_hermes_home_override()
        if value:
            env["ZYMATICA_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ZYMATICA_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_hermes_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _ZYMATICA_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX):]
    ...
    from zymatica_constants import apply_subprocess_home_env
...
# narrow subset of _ZYMATICA_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_ZYMATICA_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 2 (conditional):** the rest of ``_ZYMATICA_PROVIDER_ENV_BLOCKLIST``
    ...
    if not inherit_credentials:
        ...
        for key in _ZYMATICA_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)
    ...
    # copies os.environ, whose ZYMATICA_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's ZYMATICA_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    propagation (``ZYMATICA_HOME`` / subprocess ``HOME`` contract) and the
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      ``ZYMATICA_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Hermes home override into ``ZYMATICA_HOME`` and apply the
      subprocess HOME contract (``zymatica_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ZYMATICA_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ZYMATICA_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ZYMATICA_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale ZYMATICA_GIT_BASH_PATH pointing at one) must not
    ...
    # ZYMATICA_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ZYMATICA_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        ...
        "Or set ZYMATICA_GIT_BASH_PATH to your bash.exe location."
    )
...
_SENTINEL = object()
_ZYMATICA_BIN_DIR: "str | None | object" = _SENTINEL
...
def _resolve_hermes_bin_dir() -> str | None:
    ...
    """
    global _ZYMATICA_BIN_DIR
    if _ZYMATICA_BIN_DIR is not _SENTINEL:
        return _ZYMATICA_BIN_DIR  # type: ignore[return-value]
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    - ``$ZYMATICA_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$ZYMATICA_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    """
    try:
        from zymatica_constants import get_hermes_home, iter_hermes_node_dirs
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ZYMATICA_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hermes_internal_secret(real_key):
                continue
            run_env[real_key] = v
            ...
            if k in _ZYMATICA_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ZYMATICA_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ZYMATICA_HOME.  Using
