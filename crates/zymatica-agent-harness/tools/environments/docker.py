...
from tools.environments.local import (
    _ZYMATICA_PROVIDER_ENV_BLOCKLIST,
...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.zymatica/.env values without failing Docker command execution."""
    try:
        from zymatica_cli.config import load_env
...
def reap_orphan_containers(
    ...
    * ``label=zymatica=1`` (created by this codebase)
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    filters = ["--filter", "label=zymatica=1", "--filter", "status=exited"]
    if profile_filter:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ZYMATICA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ZYMATICA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ZYMATICA_DOCKER_BINARY override: %s", override)
        return override
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``ZYMATICA_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    # through the proxy.  Keep the ZYMATICA_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"ZYMATICA_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``zymatica:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.zymatica/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from zymatica_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from zymatica_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_ZYMATICA_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_ZYMATICA_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. zymatica:latest) already use /init as PID 1
        ...
        #   * the orphan reaper (`zymatica=1` for the global sweep filter)
        #   * future cross-process reuse (`hermes-task-id`, `hermes-profile`)
        #   * operators running `docker ps --filter label=zymatica=1`
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        forward_keys = explicit_forward_keys | (_implicit_forward - _ZYMATICA_PROVIDER_ENV_BLOCKLIST)
