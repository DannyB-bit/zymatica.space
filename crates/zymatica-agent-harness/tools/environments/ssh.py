...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.zymatica"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.zymatica directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.zymatica"
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .zymatica/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.zymatica/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.zymatica".lstrip("/")
        ssh_cmd = self._build_ssh_command()
