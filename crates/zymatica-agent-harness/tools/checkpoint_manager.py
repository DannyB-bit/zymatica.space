...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ZYMATICA_CHECKPOINT_TIMEOUT", 30)))
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "ZYMATICA_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``ZYMATICA_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ZYMATICA_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ZYMATICA_WORKDIR").write_text(
