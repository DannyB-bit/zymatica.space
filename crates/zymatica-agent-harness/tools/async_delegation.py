...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from zymatica_state import apply_wal_with_fallback
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ("scope_id", "ZYMATICA_SESSION_SCOPE_ID"),
            ("user_id", "ZYMATICA_SESSION_USER_ID"),
            ("user_name", "ZYMATICA_SESSION_USER_NAME"),
        ):
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``ZYMATICA_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``ZYMATICA_SESSION_CHAT_ID`` binding survives child
    ...
    try:
        ...
        if get_session_env("ZYMATICA_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("ZYMATICA_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
