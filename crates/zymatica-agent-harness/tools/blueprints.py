...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.zymatica.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.zymatica.blueprint`` is a mapping containing
    ...
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.zymatica.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.zymatica.blueprint`` block) they can hand
