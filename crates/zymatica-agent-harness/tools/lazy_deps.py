...
(``zymatica[all]``) and install them eagerly at setup time. That has
two problems:
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ZYMATICA_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/hermes`` read-only), setting
  ``ZYMATICA_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (ZYMATICA_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
_LAZY_TARGET_ENV = "ZYMATICA_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ZYMATICA_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ZYMATICA_DISABLE_LAZY_INSTALLS") == "1":
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # deployment sets ZYMATICA_MANAGED=true *and* ZYMATICA_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from zymatica_cli.config import get_managed_system
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``ZYMATICA_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ZYMATICA_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ZYMATICA_LAZY_INSTALL_TARGET``, installs are redirected to the writable
