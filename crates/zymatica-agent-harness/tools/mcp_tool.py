...
transport, discovers their tools, and registers them into the zymatica
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.zymatica/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.zymatica/logs/mcp-stderr.log), tagged with the
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    """
    try:
        from zymatica_cli.env_loader import get_secret_source
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "ZYMATICA_HOME", os.path.join(os.path.expanduser("~"), ".zymatica")
                )
            )
            candidates = [
...
class ElicitationHandler:
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (ZYMATICA_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ZYMATICA_SESSION_PLATFORM, so the
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"file under ~/.zymatica/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ZYMATICA_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ZYMATICA_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.zymatica/.env`` loaded at startup).
    """
    try:
