...
LOCAL_STT_COMMAND_ENV = "ZYMATICA_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ZYMATICA_LOCAL_STT_LANGUAGE"
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ZYMATICA_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``ZYMATICA_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ZYMATICA_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ZYMATICA_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from zymatica_cli.plugins import has_hook, invoke_hook
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``ZYMATICA_LOCAL_STT_LANGUAGE`` (env).
