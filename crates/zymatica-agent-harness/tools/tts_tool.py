...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.zymatica/config.yaml``. Hermes
...
Configuration is loaded from ~/.zymatica/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from zymatica_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.zymatica/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.zymatica/config.yaml.
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ZYMATICA_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from zymatica_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import zymatica_cli as _hermes_cli
        ...
        headers["X-Goog-Api-Client"] = f"zymatica/{_hermes_version}"
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.zymatica/cache/piper-voices/`` under the active
    ZYMATICA_HOME so voice downloads follow profile boundaries.
    """
    from zymatica_constants import get_hermes_dir
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("ZYMATICA_SESSION_PLATFORM", "").lower()
