...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.zymatica",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ZYMATICA_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ZYMATICA_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because ZYMATICA_HOME is exactly where
    ...
    # Reject absolute paths — they bypass the ZYMATICA_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ZYMATICA_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape ZYMATICA_HOME.
    ...
    # inside ZYMATICA_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.zymatica/{rel}"
                        result.append({
...
# Each tuple is (new_subpath, old_name) matching zymatica_constants.get_hermes_dir().
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.zymatica``,
    SSH ``<remote_home>/.zymatica``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.zymatica`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.zymatica`` is shell-expanded by the remote shell, so tool
    ...
    * singularity — NOT translated: Apptainer auto-binds the host home, so
    ...
    if backend in ("docker", "modal"):
        pass  # /root/.zymatica default
    elif backend in ("ssh", "daytona", "vercel_sandbox"):
        container_base = "~/.zymatica"
    else:
