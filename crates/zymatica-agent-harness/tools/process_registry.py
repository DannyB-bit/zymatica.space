...
def _is_supervised_gateway_process() -> bool:
    ...
    Both supervisor markers and ``_ZYMATICA_GATEWAY`` are inherited by every
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from zymatica_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.zymatica/processes.json with the raw command
