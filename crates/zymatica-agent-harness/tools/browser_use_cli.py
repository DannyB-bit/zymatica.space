...
def _managed_bin_dir() -> Optional[str]:
    """Hermes' own bin dir ($ZYMATICA_HOME/bin) — where install.sh puts uv/uvx
...
def _find_cli() -> Optional[List[str]]:
    ...
    $ZYMATICA_HOME/bin); falls back to running it through uvx (PATH, then
    managed). The managed probes matter because Hermes bootstraps its own
    uv into $ZYMATICA_HOME/bin, which is not on the user's PATH.
    """
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    into ``$ZYMATICA_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
