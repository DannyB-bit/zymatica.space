...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.zymatica/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.zymatica/.env`` first, then ``os.environ``.
    ...
    """
    try:
        from zymatica_cli.config import get_env_value as _hermes_get_env_value
    except ImportError:
...
def resolve_xai_http_credentials(
    *,
    ...
    stored in ``~/.zymatica/.env`` (the standard Hermes location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("ZYMATICA_XAI_BASE_URL")
