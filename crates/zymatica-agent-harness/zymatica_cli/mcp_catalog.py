...
Catalog policy:
- Entries are added only by merging a PR into zymatica. Presence in the
...
- Secrets prompted at install time go to ``~/.zymatica/.env`` (the
...
from zymatica_constants import get_hermes_home, get_optional_mcps_dir
...
from zymatica_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.zymatica/.env.
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from zymatica_cli.mcp_config import _env_key_for_server
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.zymatica/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.zymatica/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from zymatica_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from zymatica_cli.mcp_config import _probe_single_server
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from zymatica_cli.mcp_config import _save_mcp_server
