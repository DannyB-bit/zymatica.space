...
- Keep data: Remove code but keep ~/.zymatica/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zymatica or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hermes Agent" comment and following line
                if '# Hermes Agent' in line or '# zymatica' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        ...
        Path.home() / ".local" / "bin" / "zymatica",
        Path("/usr/local/bin/hermes"),
        Path("/usr/local/bin/hermes-acp"),
        Path("/usr/local/bin/zymatica"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'zymatica' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from zymatica_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zymatica_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZYMATICA_HOME`` and ``ZYMATICA_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # all get swept.  Also match the bare zymatica install dir.
    markers = [root + "\\zymatica", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZYMATICA_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete ZYMATICA_HOME and ZYMATICA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZYMATICA_HOME", "ZYMATICA_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZYMATICA_HOME directory.
    ...
    current ZYMATICA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's ZYMATICA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$ZYMATICA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from zymatica_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Hermes agent at {hermes_home / 'zymatica'}")
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zymatica/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zymatica/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ZYMATICA_HOMEs
    ...
    # full wipe (code + ~/.zymatica data); otherwise keep-data. Named profiles
    ...
    # their alias wrappers, and wiping their ZYMATICA_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    wipe ``$ZYMATICA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        log_info("Removing ZYMATICA_HOME / ZYMATICA_GIT_BASH_PATH User env vars...")
    ...
    #     but the packaged app + Electron userData live OUTSIDE ZYMATICA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zymatica_cli.gui_uninstall import uninstall_gui
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zymatica/, just remove the zymatica subdir
    ...
    #     under ZYMATICA_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.zymatica/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZYMATICA_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://zymatica.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://zymatica.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    ...
    so ``import zymatica_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zymatica_constants`` + ``hermes_cli.colors``
