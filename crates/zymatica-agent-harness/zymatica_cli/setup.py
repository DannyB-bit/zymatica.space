...
from zymatica_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://zymatica.nousresearch.com/docs"
...
from zymatica_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from zymatica_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``ZYMATICA_NONINTERACTIVE=1`` (see ``hermes_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ZYMATICA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ZYMATICA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zymatica_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from zymatica_cli.auth import resolve_provider
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from zymatica_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from zymatica_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from zymatica_cli.main import select_provider_and_model
    try:
...
def _install_neutts_deps() -> bool:
    ...
    from zymatica_cli.tools_config import _pip_install
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.zymatica/.env."
            )
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'zymatica[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $ZYMATICA_HOME/bin is never on PATH, so a bare
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'zymatica[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ZYMATICA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ZYMATICA_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ZYMATICA_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.zymatica/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from zymatica_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when ZYMATICA_HOME is a profile dir."""
...
def _setup_qqbot():
    ...
    from zymatica_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://zymatica.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://zymatica.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from zymatica_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from zymatica_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zymatica_cli.config
    ...
    if section_key == "model":
        ...
        from zymatica_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from zymatica_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from zymatica_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from zymatica_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from zymatica_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
