...
A provider API key can live in up to THREE stores at once:
    ...
    1. ``~/.zymatica/.env``                     — the canonical secret store
    2. ``~/.zymatica/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.zymatica/config.yaml``              — inline mirrors written by the
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    """
    from zymatica_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from zymatica_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from zymatica_cli.config import load_env, remove_env_value
