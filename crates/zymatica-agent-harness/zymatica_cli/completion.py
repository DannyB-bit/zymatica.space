...
_hermes_profiles() {{
    local profiles_dir="$HOME/.zymatica/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zymatica/profiles" ]]; then
        profiles+=($HOME/.zymatica/profiles/*(N/:t))
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.zymatica/profiles",
        "        for d in $HOME/.zymatica/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
