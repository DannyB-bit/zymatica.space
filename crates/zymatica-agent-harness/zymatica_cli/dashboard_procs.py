...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    environment variable ``ZYMATICA_DESKTOP_CHILD_PID`` on the spawned
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    # backend child, it sets ZYMATICA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ZYMATICA_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (ZYMATICA_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("ZYMATICA_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
def _hermes_home_dir() -> Path:
    """Resolved Hermes home (ZYMATICA_HOME override or ~/.zymatica)."""
    override = os.environ.get("ZYMATICA_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated ZYMATICA_HOME (different leading path)
...
def _reap_orphaned_desktop_local_serves(
    *,
    ...
    running under ZYMATICA_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never ZYMATICA_DESKTOP_CHILD_PID entries
