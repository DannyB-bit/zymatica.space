...
# ---------------------------------------------------------------------------
# Argparse wiring — called from zymatica_cli.main
# ---------------------------------------------------------------------------
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        # provider keys only in ~/.zymatica/.env (loaded automatically when the
        ...
        if loaded:
            console.print(
                ...
                f"~/.zymatica/.env for discovery.[/dim]"
            )
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.zymatica/.env`` into ``os.environ``.

    ``hermes egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.zymatica/.env`` (which the agent
