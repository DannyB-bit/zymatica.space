...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with zymatica;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.zymatica/plugins/<name>/``
3. **Project plugins** – ``./.zymatica/plugins/<name>/`` (opt-in via
   ``ZYMATICA_ENABLE_PROJECT_PLUGINS``)
...
from zymatica_cli.config import cfg_get, load_config_readonly
from zymatica_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from zymatica_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    plugin_capability_granted,
)
from zymatica_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ZYMATICA_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ZYMATICA_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ZYMATICA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.zymatica/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ZYMATICA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ZYMATICA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "ZYMATICA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ZYMATICA_EVENT_NAMESPACE = "hermes"
...
class PluginManifest:
    ...
    #              in ~/.zymatica/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from zymatica_cli import config as config_mod
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from zymatica_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``ZYMATICA_HOME`` via
        ...
        running under ``~/.zymatica/profiles/<name>``, or ``"custom"`` when
        ``ZYMATICA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        from zymatica_cli.dashboard_auth.registry import restore_registration
        from zymatica_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.zymatica/.env`` loads, before Hermes reads credentials — when
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        from zymatica_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, ZYMATICA_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{ZYMATICA_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def register_skill(
        ...
        ``~/.zymatica/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("ZYMATICA_SAFE_MODE"):
                logger.info("ZYMATICA_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from zymatica_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from zymatica_cli.approval_transport import RegisteredApprovalTransport
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.zymatica/plugins/)
        ...
        # 3. Project plugins (./.zymatica/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("ZYMATICA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".zymatica" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ZYMATICA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("ZYMATICA_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from zymatica_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from zymatica_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from zymatica_cli.agent_plugins import load_agent_plugin
...
# multiple profiles via different ZYMATICA_HOME directories, and a single
...
# zymatica_constants.set_hermes_home_override). A process-wide single-slot
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``ZYMATICA_HOME`` or the context-local
...
def _get_pre_tool_call_directive_details(
    ...
    from zymatica_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
