...
2. Python deps declared by user-installed plugins under ``~/.zymatica/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.zymatica/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from zymatica_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
