...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ZYMATICA_HOME>/.zymatica-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``hermes-setup`` binary under ``~/.zymatica`` is only refreshed by a full
...
MARKER_NAME = ".zymatica-update-in-progress"
...
HANDOFF_PID_ENV = "ZYMATICA_UPDATE_HANDOFF_PID"
...
def update_marker_path() -> Path:
    ...
    the Rust updater resolves ``$ZYMATICA_HOME`` or the platform default, and the
    ...
    """
    from zymatica_constants import get_process_hermes_home
