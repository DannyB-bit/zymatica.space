...
# when zymatica_constants is cached from a pre-update version during `hermes update`.
...
from zymatica_cli.colors import Colors, color
...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the ZYMATICA_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. ZYMATICA_HOME
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from zymatica_cli.profiles import list_profiles
    except Exception:
        return processes
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ZYMATICA_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from zymatica_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (ZYMATICA_HOME,
    ...
    if sys.platform == "win32":
        try:
            from zymatica_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ZYMATICA_HOME) the windowless
        # base interpreter needs to import zymatica_cli.  Empty on POSIX, where
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ZYMATICA_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("ZYMATICA_HOME="):
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ZYMATICA_HOME``.

    Under ``sudo``, ``ZYMATICA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.zymatica`` — the wrong
    profile. The unit file pins ``ZYMATICA_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ZYMATICA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ZYMATICA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ZYMATICA_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from zymatica_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from zymatica_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ZYMATICA_HOME-scoped).
...
from zymatica_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``ZYMATICA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ZYMATICA_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ZYMATICA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ZYMATICA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ZYMATICA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ZYMATICA_HOME is a named profile.

    For ``~/.zymatica/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit ZYMATICA_HOME path. Defaults to the current
...
def _profile_arg_for_target_user(hermes_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".zymatica"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ZYMATICA_HOME.

    Default ``~/.zymatica`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.zymatica/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other ZYMATICA_HOME appends a short hash for uniqueness.
    """
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from zymatica_cli import gateway_windows
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.zymatica`` → ``ai.zymatica.gateway.plist`` (backward compatible).
    Profile ``~/.zymatica/profiles/coder`` → ``ai.zymatica.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.zymatica.gateway-{suffix}" if suffix else "ai.zymatica.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # zymatica_constants cached from before this symbol existed. See
            # _reload_zymatica_constants() in hermes_cli/managed_uv.py.
            from zymatica_cli.managed_uv import _reload_zymatica_constants

            venv_python_path = _reload_zymatica_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.zymatica/zymatica  -> /home/alice/.zymatica/zymatica
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ZYMATICA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.zymatica                    → /home/alice/.zymatica
      /root/.zymatica/profiles/coder     → /home/alice/.zymatica/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes_raw = os.environ.get("ZYMATICA_HOME", "").strip()
    current_hermes = (
    ...
    # different ZYMATICA_HOME into the generated service unit.
    current_default = Path.home() / ".zymatica"
    target_default = Path(target_home_dir) / ".zymatica"

    # Default ~/.zymatica → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.zymatica → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.zymatica) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    ``ZYMATICA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ZYMATICA_HOME
...
def _append_node_dir_for_service(
    ...
    The Hermes-managed Node under ``$ZYMATICA_HOME/node`` goes first when it
    ...
    *hermes_root* is the Hermes home the unit will run against. System units
    ...
    """
    from zymatica_constants import iter_hermes_node_dirs
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's ZYMATICA_HOME (stable, always exists)
...
Environment="ZYMATICA_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ZYMATICA_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hermes gateway … --system``, ZYMATICA_HOME is often stripped
    # and falls back to ``/root/.zymatica``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ZYMATICA_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ZYMATICA_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ZYMATICA_HOME
    ...
    exported a throwaway ``ZYMATICA_HOME=/tmp/...`` — writing it to the real
    ...
    with ``ZYMATICA_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ZYMATICA_HOME=..."`` lines and launchd
    ``<key>ZYMATICA_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ZYMATICA_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ZYMATICA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ZYMATICA_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ZYMATICA_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ZYMATICA_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ZYMATICA_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only ZYMATICA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ZYMATICA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="ZYMATICA_HOME=..."``. Writing that to the developer's
    ...
    # ZYMATICA_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("ZYMATICA_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ZYMATICA_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Existing system units already pin ZYMATICA_HOME; adopt it before any
    ...
    # ``sudo hermes gateway install --system --force`` would bake /root/.zymatica
...
def systemd_start(system: bool = False):
    ...
    # ZYMATICA_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.zymatica.gateway-{suffix}" if suffix else "ai.zymatica.gateway"
...
def _gateway_run_command() -> list[str]:
    ...
    Profile-aware: honors the active ZYMATICA_HOME via `_profile_arg()` so the
...
def _spawn_detached_gateway() -> bool:
    ...
    """
    from zymatica_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from zymatica_constants import display_hermes_home as _dhh
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from zymatica_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.zymatica/logs/launchd-reload.log, which the health watchdog
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ZYMATICA_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from zymatica_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other ZYMATICA_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ZYMATICA_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$ZYMATICA_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ZYMATICA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Service-style launchers set ZYMATICA_GATEWAY_DETACHED=1; older wrappers
    ...
    # ZYMATICA_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ZYMATICA_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``ZYMATICA_GATEWAY_MAX_STARTS`` /
    # ``ZYMATICA_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ZYMATICA_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ZYMATICA_GATEWAY_START_WINDOW_S")
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.zymatica/plugins/ still require
...
def _setup_standard_platform(platform: dict):
    ...
    from zymatica_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from zymatica_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.zymatica/.env."
    )
    print_info(
...
def _builtin_setup_fn(key: str):
    ...
    """
    from zymatica_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.zymatica/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.zymatica/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    """
    from zymatica_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ZYMATICA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``ZYMATICA_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``ZYMATICA_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ZYMATICA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ZYMATICA_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ZYMATICA_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ZYMATICA_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ZYMATICA_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.zymatica/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from zymatica_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        if os.getenv("_ZYMATICA_GATEWAY") == "1":
            print_error(
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.zymatica/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
