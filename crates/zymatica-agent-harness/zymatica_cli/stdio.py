...
def configure_windows_stdio() -> bool:
    ...
    Set ``ZYMATICA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ZYMATICA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    candidate_dirs = [
        ...
        os.path.join(local_appdata, "hermes", "zymatica", "venv", "Scripts"),
