...
Hermes owns its own uv binary at ``$ZYMATICA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ZYMATICA_HOME/bin`` so the installer writes directly there — no PATH
...
uses an install-scoped store under ``<checkout>/.zymatica-runtime/python``. A
...
from zymatica_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".zymatica-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    ...
    ``$ZYMATICA_HOME/bin/uv`` on POSIX, ``$ZYMATICA_HOME\\bin\\uv.exe`` on
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    and, offline, an unbounded hang risk. A stamp file under ZYMATICA_HOME
...
def _reload_zymatica_constants():
    """Re-execute ``zymatica_constants`` from disk and return the fresh module.

    ``hermes update`` imports ``zymatica_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'zymatica_constants'
        (~/.zymatica/zymatica/zymatica_constants.py)
    ...
    """
    import zymatica_constants

    return importlib.reload(zymatica_constants)
...
def _install_uv(target: Path) -> None:
    ...
    ``$ZYMATICA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
