...
# ZYMATICA_HOME paths we've already pulled external secrets for during this
...
def _known_hermes_env_keys() -> set[str]:
    ...
    """
    from zymatica_cli.config import _EXTRA_ENV_KEYS
    from zymatica_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        local_env["ZYMATICA_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which ZYMATICA_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from zymatica_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_zymatica_dotenv(
    *,
    zymatica_home: str | os.PathLike | None = None,
    hermes_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    **kwargs,
) -> list[Path]:
    """Load Zymatica environment files with user config taking precedence."""
    home = zymatica_home if zymatica_home is not None else hermes_home
    return load_hermes_dotenv(hermes_home=home, project_env=project_env, **kwargs)
...
def load_hermes_dotenv(
    ...
    **kwargs,
    ...
    Behavior:
    - `~/.zymatica/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("ZYMATICA_HOME", Path.home() / ".zymatica"))
    ...
    #   EnvironmentFile=-/path/to/.zymatica/.op.env
    ...
    # TERMINAL_ENV=docker left in ~/.zymatica/.env (e.g. written by an older
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Scoped to the process ZYMATICA_HOME: the shared bridge reads the
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ZYMATICA_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    owns the once-per-ZYMATICA_HOME guard, the post-apply ASCII
...
def _process_hermes_home() -> Path:
    """The ZYMATICA_HOME the shared config cache is keyed to."""
    try:
