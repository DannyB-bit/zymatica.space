...
from zymatica_cli.doctor import (
    _section,
    check_fail,
    check_info,
    check_ok,
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from zymatica_cli.doctor import ZYMATICA_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (ZYMATICA_HOME / "node" / "bin",
                          ZYMATICA_HOME / "node",
                          ZYMATICA_HOME / "node_modules" / ".bin"):
