...
from zymatica_constants import INDICATOR_STYLES
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zymatica/skills/ for newly installed or removed skills",
...
_SLACK_VIA_ZYMATICA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ZYMATICA_ONLY:
            # Intentionally Slack-via-/hermes only (see _SLACK_VIA_ZYMATICA_ONLY).
            return
...
def slack_app_manifest(request_url: str = "https://zymatica.local/slack/commands") -> dict[str, Any]:
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zymatica_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
