...
from zymatica_cli._subprocess_compat import suppress_platform_ver_console
...
from zymatica_cli import _startup_fast  # noqa: E402
...
# ``from zymatica_cli.env_loader import ...`` / ``from zymatica_cli.config import
...
from zymatica_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from zymatica_cli.oneshot import run_oneshot
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ZYMATICA_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".zymatica", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ZYMATICA_TUI=1``, then a real-TTY gate (a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("ZYMATICA_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``ZYMATICA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ZYMATICA_TUI_NO_EARLY_DISABLE") == "1":
        return
...
from zymatica_cli.sessions_cmd import cmd_sessions  # noqa: F401
from zymatica_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from zymatica_cli.subcommands.cron import build_cron_parser
from zymatica_cli.subcommands.sync import build_sync_parser
from zymatica_cli.subcommands.gateway import build_gateway_parser
from zymatica_cli.subcommands.profile import build_profile_parser
from zymatica_cli.subcommands.model import build_model_parser
from zymatica_cli.subcommands.setup import build_setup_parser

from zymatica_cli.subcommands.whatsapp import build_whatsapp_parser
from zymatica_cli.subcommands.slack import build_slack_parser
from zymatica_cli.subcommands.login import build_login_parser
from zymatica_cli.subcommands.logout import build_logout_parser
from zymatica_cli.subcommands.auth import build_auth_parser
from zymatica_cli.subcommands.status import build_status_parser
from zymatica_cli.subcommands.pause import build_pause_parser
from zymatica_cli.subcommands.webhook import build_webhook_parser
from zymatica_cli.subcommands.hooks import build_hooks_parser
from zymatica_cli.subcommands.doctor import build_doctor_parser
from zymatica_cli.subcommands.verify import build_verify_parser
from zymatica_cli.subcommands.security import build_security_parser
from zymatica_cli.subcommands.approvals import build_approvals_parser
from zymatica_cli.subcommands.dump import build_dump_parser
from zymatica_cli.subcommands.debug import build_debug_parser
from zymatica_cli.subcommands.backup import build_backup_parser
from zymatica_cli.subcommands.import_cmd import build_import_cmd_parser
from zymatica_cli.subcommands.import_agent import build_import_agent_parser
from zymatica_cli.subcommands.config import build_config_parser
from zymatica_cli.subcommands.skin import build_skin_parser
from zymatica_cli.subcommands.console import build_console_parser
from zymatica_cli.subcommands.version import build_version_parser
from zymatica_cli.subcommands.update import build_update_parser
from zymatica_cli.subcommands.uninstall import build_uninstall_parser
from zymatica_cli.subcommands.dashboard import build_dashboard_parser
from zymatica_cli.subcommands.gui import build_gui_parser
from zymatica_cli.subcommands.logs import build_logs_parser
from zymatica_cli.subcommands.prompt_size import build_prompt_size_parser
from zymatica_cli.subcommands.memory import build_memory_parser
from zymatica_cli.subcommands.acp import build_acp_parser
from zymatica_cli.subcommands.tools import build_tools_parser
from zymatica_cli.subcommands.insights import build_insights_parser
from zymatica_cli.subcommands.monitoring import build_monitoring_parser
from zymatica_cli.subcommands.skills import build_skills_parser
from zymatica_cli.subcommands.pairing import build_pairing_parser
from zymatica_cli.subcommands.plugins import build_plugins_parser
from zymatica_cli.subcommands.mcp import build_mcp_parser
from zymatica_cli.subcommands.claw import build_claw_parser
...
#
# Many modules cache ZYMATICA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZYMATICA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zymatica/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZYMATICA_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".zymatica" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # 1.5 If ZYMATICA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zymatica/profiles/coder or
    # /opt/data/profiles/coder).  If ZYMATICA_HOME points to the hermes root
    # instead (e.g. systemd hardcodes ZYMATICA_HOME=/root/.zymatica), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("ZYMATICA_HOME", "")
    ...
    # run-script as ZYMATICA_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # "the root ZYMATICA_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ZYMATICA_S6_SUPERVISED_CHILD"):
        try:
            from zymatica_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set ZYMATICA_HOME
    if profile_name is not None:
        try:
            from zymatica_cli.profiles import resolve_profile_env
            ...
            return
        os.environ["ZYMATICA_HOME"] = hermes_home
...
# Load .env from ~/.zymatica/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zymatica_cli.config import get_hermes_home
from zymatica_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZYMATICA_REDACT_SECRETS env
...
try:
    ...
    from zymatica_cli.config import read_raw_config as _read_raw_early
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ZYMATICA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ZYMATICA_REDACT_SECRETS"] = str(_early_redact).lower()
...
if _FORCE_IPV4_EARLY:
    try:
        from zymatica_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if zymatica_constants not importable yet
...
from zymatica_cli import __version__, __release_date__
...
from zymatica_cli.model_setup_flows import (
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ZYMATICA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ZYMATICA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    """
    from zymatica_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from zymatica_cli.config import get_env_path, get_hermes_home, load_config
    from zymatica_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zymatica_cli.config import DEFAULT_CONFIG
    ...
    from zymatica_cli.auth import PROVIDER_REGISTRY
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ZYMATICA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ZYMATICA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ZYMATICA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZYMATICA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    from zymatica_constants import get_hermes_home
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZYMATICA_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ZYMATICA_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZYMATICA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZYMATICA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $ZYMATICA_HOME/node tree,
        ...
        from zymatica_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from zymatica_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ZYMATICA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZYMATICA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZYMATICA_TUI_DIR (e.g. `unset ZYMATICA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from zymatica_constants import with_hermes_node_path
        ...
        if result.returncode != 0:
            ...
            from zymatica_cli.npm_engine import maybe_repair_npm_engine
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zymatica_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ZYMATICA_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ZYMATICA_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ZYMATICA_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ZYMATICA_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ZYMATICA_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ZYMATICA_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ZYMATICA_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    try:
        from zymatica_cli.config import apply_terminal_config_to_env
    ...
    env["ZYMATICA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ZYMATICA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["ZYMATICA_MODEL"] = model
        env["ZYMATICA_INFERENCE_MODEL"] = model
    if provider:
        env["ZYMATICA_TUI_PROVIDER"] = provider
        env["ZYMATICA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZYMATICA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZYMATICA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZYMATICA_TUI_SKILLS"] = value
    if query:
        env["ZYMATICA_TUI_QUERY"] = query
    if image:
        env["ZYMATICA_TUI_IMAGE"] = image
    if checkpoints:
        env["ZYMATICA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZYMATICA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZYMATICA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZYMATICA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZYMATICA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZYMATICA_ACCEPT_HOOKS"] = "1"
    ...
    # ZYMATICA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set ZYMATICA_TUI_RESUME themselves.
    env.pop("ZYMATICA_TUI_RESUME", None)
    if resume_session_id:
        env["ZYMATICA_TUI_RESUME"] = resume_session_id
    ...
    if code == 42:
        from zymatica_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZYMATICA_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("ZYMATICA_KANBAN_BOARD"):
        return
    try:
        from zymatica_cli.kanban_db import get_current_board

        os.environ["ZYMATICA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zymatica/skills/`` with the bundled skill library on first launch.
...
def _resolve_use_tui(args) -> bool:
    ...
    try:
        ...
        return False
    if os.environ.get("ZYMATICA_TUI") == "1":
        return True
    try:
        from zymatica_cli.config import load_config
...
def cmd_chat(args):
    ...
    try:
        from zymatica_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from zymatica_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        ...
        from zymatica_cli.setup import (
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zymatica_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZYMATICA_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zymatica/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZYMATICA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZYMATICA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZYMATICA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zymatica_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from zymatica_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zymatica_cli.config import get_env_value, save_env_value
    from zymatica_constants import find_node_executable, with_hermes_node_path
...
def cmd_whatsapp_cloud(args):
    ...
    from zymatica_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from zymatica_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from zymatica_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from zymatica_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from zymatica_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from zymatica_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("ZYMATICA_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zymatica_cli.config import read_raw_config
    ...
    from zymatica_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        _PROVIDER_ALIASES,
        group_providers,
    ...
    # a leftover OPENAI_BASE_URL in ~/.zymatica/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zymatica/.env if the active provider is not 'custom'.
    ...
    """
    from zymatica_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    try:
        from zymatica_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from zymatica_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from zymatica_cli.config import load_config
    from zymatica_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from zymatica_cli.auth import _prompt_model_selection
    from zymatica_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from zymatica_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zymatica_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from zymatica_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from zymatica_cli.curses_ui import curses_radiolist
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zymatica_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    recover from a malformed paste without editing ``~/.zymatica/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from zymatica_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from zymatica_cli.config import save_env_value
    from zymatica_cli.secret_prompt import masked_secret_prompt
    ...
    from zymatica_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zymatica_constants import display_hermes_home as _dhh_fn
...
def cmd_login(args):
    ...
    from zymatica_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zymatica_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zymatica_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zymatica_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zymatica_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.zymatica/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set ZYMATICA_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "ZYMATICA_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
...
def cmd_webhook(args):
    ...
    from zymatica_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from zymatica_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zymatica_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from zymatica_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from zymatica_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zymatica_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from zymatica_cli.verify_cmd import run_verify_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from zymatica_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from zymatica_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from zymatica_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zymatica_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zymatica_cli.config import config_command
...
def cmd_skin(args):
    ...
    from zymatica_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from zymatica_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zymatica_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from zymatica_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from zymatica_cli.config import detect_install_method
    from zymatica_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from zymatica_cli.banner import check_for_updates
        from zymatica_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from zymatica_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zymatica_cli.uninstall import run_gui_uninstall
    ...
    from zymatica_cli.uninstall import run_uninstall
...
from zymatica_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in ZYMATICA_HOME) because
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ZYMATICA_HOME."""
    from zymatica_constants import get_hermes_home
...
#
# Stamp file: $ZYMATICA_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ZYMATICA_HOME."""
    from zymatica_constants import get_hermes_home
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the ZYMATICA_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    ...
    """
    try:
        from zymatica_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["ZYMATICA_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ZYMATICA_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "hermes_root", None):
        env["ZYMATICA_DESKTOP_ZYMATICA_ROOT"] = str(Path(args.zymatica_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ZYMATICA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ZYMATICA_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ZYMATICA_DESKTOP_DISABLE_GPU=... hermes desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ZYMATICA_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ZYMATICA_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `ZYMATICA_DESKTOP_PASSWORD_STORE=... hermes desktop` keeps working.
    if sys.platform == "linux" and "ZYMATICA_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["ZYMATICA_DESKTOP_PASSWORD_STORE"] = password_store
...
from zymatica_cli.dashboard_procs import (  # noqa: F401
...
# Install-scoped breadcrumbs live next to the venv (not under $ZYMATICA_HOME)
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from zymatica_cli.managed_uv import ensure_uv
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from zymatica_constants import venv_bin_dir
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"hermes", "zymatica", "hermes-acp"}
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    skips a launcher write. The symptom is ``zymatica.exe`` and
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from zymatica_constants import venv_python_path
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.zymatica/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zymatica/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        from zymatica_cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    ...
    from zymatica_cli.update_lock import (
...
def cmd_profile(args):
    ...
    from zymatica_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    from zymatica_constants import display_hermes_home
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.zymatica)")
            else:
        ...
        from zymatica_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zymatica_constants import get_hermes_home as _hh
        ...
        from zymatica_cli import profile_describer as _pd
        ...
        from zymatica_cli.profiles import profile_exists, validate_alias_name
        ...
        from zymatica_cli.profiles import rename_profile
        ...
        from zymatica_cli.profiles import export_profile
        ...
        from zymatica_cli.profiles import import_profile
        ...
        import tempfile
        from zymatica_cli.profile_distribution import (
            plan_install,
        ...
        from zymatica_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from zymatica_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from zymatica_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zymatica_requires:
        print(f"  Requires: Hermes {mf.zymatica_requires}")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from zymatica_cli.web_server import should_require_auth
    ...
    try:
        from zymatica_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    if choice == "2":
        ...
        print(
            ...
            "ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID into ~/.zymatica/.env for you.\n"
            "  Docs: https://zymatica.nousresearch.com/docs/"
    ...
    try:
        from zymatica_cli.config import load_config, save_config
        from zymatica_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from zymatica_cli.plugins import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from zymatica_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.zymatica/desktop-ssh: a
    # literal "~/.zymatica/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of ZYMATICA_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_hermes_home(): a non-default sticky profile (or any ZYMATICA_HOME pointing
    ...
    token_root = _Path.home() / ".zymatica" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ZYMATICA_WEB_DIST`` to ``.../app.asar/dist`` or
...
def cmd_dashboard(args):
    ...
    # Desktop Electron spawns its backend with ZYMATICA_DESKTOP=1 plus
    # ZYMATICA_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # ZYMATICA_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `hermes dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ZYMATICA_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ZYMATICA_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ZYMATICA_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ZYMATICA_SERVE_HEADLESS below.
    if os.environ.get("ZYMATICA_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ZYMATICA_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("ZYMATICA_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("ZYMATICA_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → ZYMATICA_HOME points into
    ...
    try:
        from zymatica_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ZYMATICA_DESKTOP") != "1"
    ):
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ZYMATICA_HOME is explicitly pinned to the
        ...
        # ZYMATICA_HOME.  We must resolve the root explicitly instead of just
        # dropping ZYMATICA_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ZYMATICA_HOME=/opt/data`), so an unset
        # ZYMATICA_HOME falls back to $HOME/.zymatica = /opt/data/.zymatica — an
        ...
        # returns the root for both layouts: ~/.zymatica for a standard install
        ...
        try:
            from zymatica_constants import get_default_hermes_root
            env["ZYMATICA_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ZYMATICA_HOME) rather than block the reroute.
            env.pop("ZYMATICA_HOME", None)
    ...
    from zymatica_cli.resource_limits import apply_nofile_soft_limit
    ...
    if _headless_backend:
        ...
        os.environ["ZYMATICA_SERVE_HEADLESS"] = "1"
    elif "ZYMATICA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZYMATICA_WEB_DIST"])
            if "ZYMATICA_WEB_DIST" in os.environ
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ZYMATICA_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ZYMATICA_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ZYMATICA_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ZYMATICA_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ZYMATICA_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ZYMATICA_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ZYMATICA_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ZYMATICA_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ZYMATICA_WEB_DIST: {_dist_root}")
    ...
    try:
        from zymatica_cli.mcp_startup import start_background_mcp_discovery
    ...
    from zymatica_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from zymatica_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from zymatica_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zymatica_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zymatica_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from zymatica_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from zymatica_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zymatica_cli.models import CANONICAL_PROVIDERS as _cp
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ZYMATICA_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ZYMATICA_YOLO_MODE is set before ANY
    ...
    try:
        from zymatica_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["ZYMATICA_SAFE_MODE"] = "1"
    os.environ["ZYMATICA_IGNORE_USER_CONFIG"] = "1"
    os.environ["ZYMATICA_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("ZYMATICA_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from zymatica_cli.config import get_container_exec_info
    ...
    from zymatica_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("ZYMATICA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ZYMATICA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ZYMATICA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ZYMATICA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from zymatica_constants import get_hermes_home, display_hermes_home
        ...
        from zymatica_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zymatica_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zymatica_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zymatica_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: zymatica[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zymatica_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zymatica_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zymatica_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zymatica_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from zymatica_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from zymatica_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``zymatica`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``ZYMATICA_AGENT`` is the Hermes-specific marker. setdefault: never
    clobber an outer harness (e.g. Hermes running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "zymatica")
    os.environ.setdefault("ZYMATICA_AGENT", "true")
...
def main():
    ...
    try:
        from zymatica_cli.stdio import configure_windows_stdio
    ...
    from zymatica_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from zymatica_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://zymatica.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.zymatica/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://zymatica.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from zymatica_cli import secrets_cli as _secrets_cli
    from zymatica_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://zymatica.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from zymatica_cli import proxy_cli as _proxy_cli
    ...
    # migrate command
    # =========================================================================
    from zymatica_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from zymatica_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from zymatica_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from zymatica_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from zymatica_cli.projects_cmd import build_parser as _build_project_parser
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zymatica/checkpoints/",
    ...
    from zymatica_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_import_agent(args):
        from zymatica_cli.agent_import import import_agent_command
    ...
    from zymatica_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zymatica_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zymatica_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from zymatica_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from zymatica_cli.journey import register_cli as _register_journey_cli
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from zymatica_cli.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from zymatica_cli.tools_config import _cua_driver_env
    ...
    # --yolo: set ZYMATICA_YOLO_MODE *before* plugin discovery.  The call to
