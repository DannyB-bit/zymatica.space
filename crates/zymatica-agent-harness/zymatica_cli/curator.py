...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.zymatica/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        "\nThis will replace the current ~/.zymatica/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.zymatica/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.zymatica/skills/ from a curator snapshot "
