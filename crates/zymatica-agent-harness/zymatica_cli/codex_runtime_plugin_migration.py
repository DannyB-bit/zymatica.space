...
MIGRATION_MARKER = (
    "# managed by zymatica — `hermes codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end zymatica managed section"
)
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_ZYMATICA_KEYS:
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ZYMATICA_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ZYMATICA_HOME pointing
    ...
    ``~/.zymatica`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    ZYMATICA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ZYMATICA_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever ZYMATICA_HOME its launcher sets
    ...
    # — that would override the launcher's ZYMATICA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ZYMATICA_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("ZYMATICA_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["ZYMATICA_HOME"] = hermes_home
    ...
    env["ZYMATICA_QUIET"] = "1"
    env["ZYMATICA_REDACT_SECRETS"] = env.get("ZYMATICA_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.zymatica_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.zymatica/config.yaml dict
