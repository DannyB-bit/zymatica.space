...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not platforms:
        ...
        print("channel discovery can populate ~/.zymatica/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.zymatica/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.zymatica/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.zymatica/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.zymatica/.env and ~/.zymatica/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.zymatica/.env + ~/.zymatica/config.yaml) — no LLM, "
