...
from zymatica_cli.auth import AuthError, resolve_provider
from zymatica_cli.colors import Colors, color
from zymatica_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from zymatica_cli.models import provider_label
...
from zymatica_cli.runtime_provider import resolve_requested_provider
...
def _estop_status_line():
    ...
    Cheap: a single stat on $ZYMATICA_HOME/ESTOP via agent.estop.
    """
    try:
...
def show_status(args):
    ...
    if _effective_provider_label() == "LM Studio":
        from zymatica_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'zymatica[vercel]')"
    ...
    try:
        from zymatica_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    try:
        from zymatica_cli.active_sessions import (
