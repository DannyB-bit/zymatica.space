...
configuration in ~/.zymatica/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from zymatica_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.zymatica/.env`` into ``os.environ`` and recursively interpolate any
...
def mcp_command(args):
    ...
    if action == "picker":
        from zymatica_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from zymatica_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from zymatica_cli.mcp_picker import install_by_name
        import sys as _sys
