...
from zymatica_cli.config import get_hermes_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.zymatica/.env.
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.zymatica_build_sha`` by the Dockerfile's
    ``ZYMATICA_GIT_SHA`` build-arg (see ``hermes_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from zymatica_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.zymatica/.env: a managed
