...
from zymatica_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/zymatica.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/zymatica"
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``ZYMATICA_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("ZYMATICA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ZYMATICA_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    try:
        from zymatica_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $ZYMATICA_HOME/zymatica/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "zymatica"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$ZYMATICA_HOME/zymatica/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from zymatica_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/zymatica/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/zymatica repo (forks don't get a link).
    """
...
def _format_update_notice(behind: int) -> str:
    ...
    from zymatica_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        from zymatica_cli.config import get_config_path
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ZYMATICA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ZYMATICA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    if os.getenv("ZYMATICA_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from zymatica_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from zymatica_cli.codex_runtime_switch import get_current_runtime
        from zymatica_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ZYMATICA_AGENT_LOGO
        console.print(_logo)
        console.print()
