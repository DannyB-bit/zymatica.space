...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured ZYMATICA_HOME spelling.
    ...
    path lives under ZYMATICA_HOME.
    """
    candidate = Path(path)
    try:
...
def _current_profile_cli_args() -> list[str]:
    ...
    from zymatica_cli.gateway import _profile_arg
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ZYMATICA_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ZYMATICA_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ZYMATICA_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ZYMATICA_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ZYMATICA_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ZYMATICA_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ZYMATICA_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ZYMATICA_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ZYMATICA_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    from zymatica_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<ZYMATICA_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ZYMATICA_HOME`` whenever it
    ...
    source checkout only if ``ZYMATICA_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ZYMATICA_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "ZYMATICA_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ZYMATICA_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports zymatica_constants-based logic during startup.
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('ZYMATICA_HOME')}) = {_quote_vbs_string(hermes_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('ZYMATICA_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    env_overlay = {
        "ZYMATICA_HOME": hermes_home,
        "PYTHONIOENCODING": "utf-8",
        "ZYMATICA_GATEWAY_DETACHED": "1",
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (ZYMATICA_HOME, VIRTUAL_ENV,
    ...
    from zymatica_cli.config import get_hermes_home
    from zymatica_cli.gateway import PROJECT_ROOT
    ...
    if hermes_home:
        env_overlay["ZYMATICA_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ZYMATICA_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ZYMATICA_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from zymatica_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from zymatica_cli.gateway import find_gateway_pids, _profile_arg
