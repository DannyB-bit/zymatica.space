...
user's ``~/.zymatica/config.yaml`` and ``~/.zymatica/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``ZYMATICA_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``ZYMATICA_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$ZYMATICA_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("ZYMATICA_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    try:
        ...
        from zymatica_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
