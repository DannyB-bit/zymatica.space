...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ZYMATICA_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ZYMATICA_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ZYMATICA_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ZYMATICA_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ZYMATICA_HOME/skills/codex-imports/<name>/
...
class AgentImporter:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ZYMATICA_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``hermes import-agent`` (invoked from zymatica_cli.main)."""
    from zymatica_cli.config import get_config_path, load_config, save_config
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'hermes setup' or ~/.zymatica/.env.")
        print()
