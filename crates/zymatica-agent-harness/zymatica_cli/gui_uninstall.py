...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$ZYMATICA_HOME/zymatica/``:
       ...
       - ``$ZYMATICA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$ZYMATICA_HOME``:
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    ...
    Python agent runs from ``zymatica/`` source + ``venv/`` and never
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from zymatica_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ZYMATICA_HOME.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``zymatica/hermes_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ZYMATICA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from zymatica_cli.linux_desktop_entry import (
                desktop_entry_path,
