...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from zymatica_cli._subprocess_compat import split_command_line
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from zymatica_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from zymatica_cli.config import show_config
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from zymatica_cli.config import set_config_value
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from zymatica_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from zymatica_cli.cron import cron_status
