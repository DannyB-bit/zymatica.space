...
`hermes backup` creates a zip archive of the entire ~/.zymatica/ directory
(excluding the zymatica repo and transient files).
...
from zymatica_constants import get_default_hermes_root, get_hermes_home, display_hermes_home

# Shared formatter; the private alias is kept because claw.py and the backup
# tests import ``_format_size`` from this module.
from zymatica_cli.sizefmt import format_bytes as _format_size
...
# ``zymatica`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/zymatica/``
...
# ZYMATICA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "zymatica",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ZYMATICA_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE ZYMATICA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ZYMATICA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``zymatica`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/zymatica/`` — must be preserved.
        if part == "zymatica" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ZYMATICA_HOME.
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``zymatica`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "zymatica" and not is_root)
        ]
    ...
    # outside ZYMATICA_HOME, so the walk above never sees it. Ask the active
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zymatica/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ZYMATICA_HOME.
        ...
        # Guidance
        print()
        if not (hermes_root / "zymatica").is_dir():
            print("Note: The zymatica codebase was not included in the backup.")
        ...
        try:
            from zymatica_cli.gateway import ensure_gateway_service, _is_service_running
...
# Critical state files to include in quick snapshots (relative to ZYMATICA_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $ZYMATICA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ZYMATICA_HOME and the entry is silently
...
# Relative path of the cron job database inside ZYMATICA_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZYMATICA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZYMATICA_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZYMATICA_HOME under ``backups/`` before a
    ...
    ``<ZYMATICA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
