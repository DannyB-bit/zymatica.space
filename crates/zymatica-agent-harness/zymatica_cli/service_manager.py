...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from zymatica_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from zymatica_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from zymatica_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from zymatica_cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from zymatica_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from zymatica_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from zymatica_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from zymatica_cli.gateway import _probe_launchd_service_running
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ZYMATICA_HOME blindly;
    derive the shared profile root from the current ZYMATICA_HOME and map the
    ...
    hermes_home = Path(os.environ.get("ZYMATICA_HOME", "/opt/data"))
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ZYMATICA_UID, _ZYMATICA_GID)
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ZYMATICA_UID, _ZYMATICA_GID)
        except PermissionError:
            pass
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ZYMATICA_UID, _ZYMATICA_GID)
            except PermissionError:
                pass
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ZYMATICA_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ZYMATICA_HOME=/data/hermes`` is honored at run
        ...
        root ZYMATICA_HOME profile" (the implicit profile that exists at
        the top of $ZYMATICA_HOME, not under profiles/). It must be
        ...
        would instead look up ``$ZYMATICA_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ZYMATICA_HOME``,
        ...
        # guard.
        lines.append("export ZYMATICA_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's ZYMATICA_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ZYMATICA_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The ZYMATICA_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own ZYMATICA_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ZYMATICA_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ZYMATICA_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ZYMATICA_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{ZYMATICA_HOME:=/opt/data}}"\n'
            f'log_dir="$ZYMATICA_HOME/logs/gateways/{prof}"\n'
