...
Saves per-platform tool configuration to ~/.zymatica/config.yaml under
...
from zymatica_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from zymatica_cli.nous_account import format_nous_portal_entitlement_message
...
from zymatica_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ZYMATICA_CORE_TOOLS (available at runtime if enabled),
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from zymatica_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from zymatica_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from zymatica_cli.plugins import get_plugin_toolset_keys_nowait
...
from zymatica_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ZYMATICA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ZYMATICA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ZYMATICA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ZYMATICA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ZYMATICA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ZYMATICA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ZYMATICA_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ZYMATICA_HOME/bin is never on PATH, so a bare which()
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        ...
        if verbose:
            ...
            # handle is ~/.zymatica/logs/update.log — write straight to it so
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/zymatica:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.zymatica/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.zymatica/config.yaml")
        ...
        # to ~/.zymatica/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from zymatica_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from zymatica_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hermes-cli" (which include all _ZYMATICA_CORE_TOOLS) cause disabled
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from zymatica_constants import hermes_home_key
...
def _agent_browser_installed() -> bool:
    ...
    from zymatica_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from zymatica_cli.nous_subscription import _has_agent_browser
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    """
    try:
        from zymatica_cli.inventory import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://zymatica.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
