...
is the session DB (``~/.zymatica/state.db``): every assistant ``terminal`` tool
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "Entries are merged into command_allowlist in ~/.zymatica/config.yaml."
    )
...
def approvals_command(args) -> int:
    ...
    if sub == "test":
        from zymatica_cli.approvals_test import approvals_test_command
