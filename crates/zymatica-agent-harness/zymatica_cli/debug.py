...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.zymatica/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.zymatica/pastes/pending.json``.
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "zymatica/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.zymatica/pastes/pending.json``
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zymatica/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zymatica_cli.logs import LOG_FILES
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from zymatica_cli.diagnostics_upload import share_to_nous
