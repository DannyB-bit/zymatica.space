...
from zymatica_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
WEB_DIST = Path(os.environ["ZYMATICA_WEB_DIST"]) if "ZYMATICA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same ZYMATICA_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from zymatica_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (ZYMATICA_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ZYMATICA_DESKTOP") == "1":
        ...
        try:
            from zymatica_cli.gateway import _reap_unsupervised_gateway_orphans
...
from zymatica_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ZYMATICA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("ZYMATICA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
# ---------------------------------------------------------------------------
from zymatica_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from zymatica_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from zymatica_cli.dashboard_auth.token_auth import token_auth_middleware
...
from zymatica_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from zymatica_cli.config import get_compatible_custom_providers
    from zymatica_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from zymatica_cli.model_normalize import normalize_model_for_provider
    from zymatica_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from zymatica_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "ZYMATICA_DASHBOARD_FILES_ROOT"
...
# ZYMATICA_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ZYMATICA_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ZYMATICA_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("ZYMATICA_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from zymatica_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when ZYMATICA_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``ZYMATICA_HOME/images/`` — the same directory ``clipboard.paste`` /
...
async def fs_write_text(payload: FsWriteText):
    ...
    tmp = target.with_name(f".{target.name}.zymatica-tmp-{os.getpid()}")
    try:
...
from zymatica_cli import web_git as _web_git  # noqa: E402
...
from zymatica_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from zymatica_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from zymatica_cli.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from zymatica_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ZYMATICA_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        auth_flows: list[str] = []
        try:
            from zymatica_cli.dashboard_auth import (
        ...
        try:
            from zymatica_cli.auth import get_nous_session_validity
        ...
        try:
            ...
            from zymatica_constants import get_hermes_home as _ghh
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from zymatica_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from zymatica_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from zymatica_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.zymatica/logs/<action>.log`` so
...
def _spawn_hermes_action(
    ...
    *,
    ...
    # _ZYMATICA_GATEWAY=1. Inheriting it makes a spawned `hermes gateway restart`
    ...
    action_env = {**os.environ, "ZYMATICA_NONINTERACTIVE": "1"}
    action_env.pop("_ZYMATICA_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("ZYMATICA_ACTION_ID")
    if action_id:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ZYMATICA_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides={"ZYMATICA_ACTION_ID": action_id},
        )
    except Exception as exc:
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.zymatica/config.yaml`` under ``tts.``.
    """
...
from zymatica_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from zymatica_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from zymatica_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from zymatica_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # (ZYMATICA_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    # Holographic stores under plugins.zymatica-memory-store.
...
async def get_egress_status():
    ...
    from zymatica_cli.proxy_cli import format_status_text
...
async def get_model_options(
    ...
    """
    try:
        from zymatica_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from zymatica_cli.auth import get_provider_auth_state
    ...
    try:
        ...
        from zymatica_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from zymatica_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from zymatica_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.zymatica/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from zymatica_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from zymatica_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from zymatica_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from zymatica_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from zymatica_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level ZYMATICA_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from zymatica_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Hermes from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Hermes over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Hermes through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Hermes over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Hermes via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://zymatica.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
...
from zymatica_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # ZYMATICA_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from zymatica_constants import get_hermes_dir
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.zymatica.nousresearch.com"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the ZYMATICA_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized ZYMATICA_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.zymatica/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
    ...
    try:
        ...
        pass
    try:
        from zymatica_cli.config import get_env_value
    except ImportError:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://zymatica.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from zymatica_cli import auth as hauth
        if provider_id == "nous":
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from zymatica_cli.auth import clear_provider_auth
            ...
            try:
                from zymatica_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.zymatica/.anthropic_oauth.json AND credential pool
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ZYMATICA_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from zymatica_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from zymatica_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from zymatica_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from zymatica_cli.logs import _read_tail, LOG_FILES
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from zymatica_cli import profiles as profiles_mod
    try:
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (ZYMATICA_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.zymatica``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    ZYMATICA_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ZYMATICA_HOME)."""
    from zymatica_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from zymatica_constants import (
...
from zymatica_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from zymatica_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # test_config_read_guard.py). The ZYMATICA_HOME override scopes
...
def _normalize_mcp_server_create(
    ...
    """
    from zymatica_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from zymatica_cli.mcp_security import validate_mcp_server_entry
...
from zymatica_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from zymatica_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from zymatica_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.zymatica_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from zymatica_constants import reset_hermes_home_override, set_hermes_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import HermesTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_hermes_home_override(flow.zymatica_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.zymatica_home)))
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        hermes_home=flow.zymatica_home,
                    )
...
async def list_webhooks():
    import zymatica_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from zymatica_cli.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    from zymatica_cli.auth import suppress_credential_source
...
async def list_hooks():
    ...
    """
    def _run():
        from zymatica_cli.config import load_config as _load_config
        from agent import shell_hooks

        try:
            from zymatica_cli.plugins import VALID_HOOKS
...
from zymatica_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from zymatica_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ZYMATICA_HOME override so the write lands in the target
    ...
    """
    from zymatica_constants import set_hermes_home_override, reset_hermes_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ZYMATICA_HOME override (same mechanism as
    ...
    """
    from zymatica_constants import set_hermes_home_override, reset_hermes_home_override
    from zymatica_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ZYMATICA_HOME override. Returns the
    number of skills newly disabled.
    """
    from zymatica_constants import set_hermes_home_override, reset_hermes_home_override
    from zymatica_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a ZYMATICA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
from zymatica_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from zymatica_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from zymatica_cli.tools_config import (
...
if sys.platform.startswith("win"):
    try:
        from zymatica_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from zymatica_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from zymatica_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from zymatica_cli.dashboard_auth.audit import AuditEvent, audit_log
        from zymatica_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``ZYMATICA_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ZYMATICA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ZYMATICA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ZYMATICA_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ZYMATICA_HOME`` at the profile dir in the child env. Every spawned
    ...
    ``ZYMATICA_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from zymatica_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # An explicit profile scope below still overrides ZYMATICA_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("ZYMATICA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ZYMATICA_TUI_INLINE", "1")
    ...
    env["ZYMATICA_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ZYMATICA_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["ZYMATICA_TUI_RESUME"] = resume

    if sidecar_url:
        env["ZYMATICA_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ZYMATICA_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ZYMATICA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ZYMATICA_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("ZYMATICA_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from zymatica_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from zymatica_cli.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("ZYMATICA_TUI_RESUME") or raw_resume
...
# ZYMATICA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from zymatica_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__ZYMATICA_BASE_PATH__`` honour that prefix
    ...
    _headless = os.environ.get("ZYMATICA_SERVE_HEADLESS") == "1"
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__ZYMATICA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ZYMATICA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZYMATICA_BASE_PATH__="{prefix}";'
                f"window.__ZYMATICA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ZYMATICA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ZYMATICA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZYMATICA_BASE_PATH__="{prefix}";'
                f"window.__ZYMATICA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``ZYMATICA_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.zymatica/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.zymatica/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``ZYMATICA_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.zymatica/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.zymatica/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.zymatica/plugins/  (only if ZYMATICA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from zymatica_cli.plugins import get_bundled_plugins_dir
    ...
    # ZYMATICA_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
    ...
    if env_var_enabled("ZYMATICA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".zymatica" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from zymatica_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zymatica_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zymatica_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zymatica_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zymatica_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.zymatica/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.zymatica/plugins/ if you trust it)",
...
from zymatica_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ZYMATICA_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ZYMATICA_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(desktop_pid: int, pid_exists=None) -> bool:
    ...
    ``ZYMATICA_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    The desktop passes its own PID via ZYMATICA_PARENT_PID. When that process
    ...
    ZYMATICA_SERVE_WATCHDOG_POLL_S.
    """
    raw = os.environ.get("ZYMATICA_PARENT_PID")
    if not raw:
        return
    try:
        desktop_pid = int(raw)
        ...
        return
    try:
        poll = max(0.5, float(os.environ.get("ZYMATICA_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ZYMATICA_SERVE_HEADLESS``), so
    ...
    try:
        from zymatica_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            # (#54489).
            try:
                from zymatica_cli.config import load_config as _load_cfg
                from zymatica_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
    ...
    # ZYMATICA_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("ZYMATICA_DESKTOP") == "1":
                try:
                    from zymatica_cli.dashboard_procs import (
            ...
            # for standalone `hermes serve` (no ZYMATICA_PARENT_PID env).
            ...
            ready_token = "ZYMATICA_BACKEND_READY" if headless else "ZYMATICA_DASHBOARD_READY"
