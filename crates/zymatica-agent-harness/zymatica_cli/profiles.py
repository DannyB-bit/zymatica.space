...
Each profile is a fully independent ZYMATICA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zymatica/profiles/<name>/`` by default.

The "default" profile is ``~/.zymatica`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See zymatica_constants.get_subprocess_home().
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.zymatica``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.zymatica) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zymatica",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ZYMATICA_HOME equals the
...
# artifacts* at the root of ZYMATICA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``zymatica_constants``.
...
_ZYMATICA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current ZYMATICA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZYMATICA_HOME points outside
    ``~/.zymatica``, profiles live under ``ZYMATICA_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) ZYMATICA_HOME path.

    In standard deployments this is ``~/.zymatica``.
    In Docker/custom deployments where ZYMATICA_HOME is outside ``~/.zymatica``
    (e.g. ``/opt/data``), returns ZYMATICA_HOME directly.
    """
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZYMATICA_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _ZYMATICA_SUBCOMMANDS:
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ZYMATICA_HOME``.
    """
    try:
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ZYMATICA_HOME. When
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zymatica)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zymatica/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    if not soul_path.exists():
        try:
            from zymatica_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZYMATICA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZYMATICA_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ZYMATICA_HOME``
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # ...or by ZYMATICA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ZYMATICA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zymatica).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ZYMATICA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from zymatica_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZYMATICA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZYMATICA_HOME")
    try:
        os.environ["ZYMATICA_HOME"] = str(profile_dir)
        from zymatica_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZYMATICA_HOME"] = old_home
        elif "ZYMATICA_HOME" in os.environ:
            del os.environ["ZYMATICA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zymatica/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZYMATICA_HOME.

    Returns ``"default"`` if ZYMATICA_HOME is not set or points to ``~/.zymatica``.
    Returns the profile name if ZYMATICA_HOME points into ``~/.zymatica/profiles/<name>``.
    Returns ``"custom"`` if ZYMATICA_HOME is set to an unrecognized path.
    """
    from zymatica_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ZYMATICA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside ZYMATICA_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``security.redact_secrets`` / ``ZYMATICA_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zymatica itself — its parent is ~/ and its
        # directory name is ".zymatica", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.zymatica itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zymatica). "
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZYMATICA_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the ZYMATICA_HOME environment variable.
    """
