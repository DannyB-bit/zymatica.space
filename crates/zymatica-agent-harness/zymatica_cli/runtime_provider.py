...
from zymatica_cli import auth as auth_mod
...
from zymatica_cli.providers import custom_provider_aliases, custom_provider_slug
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from zymatica_cli.auth import resolve_provider as _resolve_provider
...
def _fallback_api_mode(provider: str, base_url: str, model: str = "") -> str:
    ...
    if detected:
        return detected
    from zymatica_cli.providers import determine_api_mode
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from zymatica_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from zymatica_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from zymatica_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ZYMATICA_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ZYMATICA_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ZYMATICA_INFERENCE_PROVIDER", "").strip()
    ...
    *,
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.zymatica/.env or run 'hermes model' to configure. To use "
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("ZYMATICA_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("ZYMATICA_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from zymatica_cli.config import is_provider_enabled, load_config
    ...
    if requested_provider in ("vertex", "google-vertex", "vertex-ai", "gcp-vertex", "vertexai"):
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "in ~/.zymatica/.env, or run 'gcloud auth application-default "
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("ZYMATICA_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from zymatica_cli.auth import resolve_minimax_oauth_runtime_credentials
