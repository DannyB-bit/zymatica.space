...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary ZYMATICA_HOME
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                "ZYMATICA_HOME": str(home),
                "ZYMATICA_BUNDLED_PLUGINS": str(bundled),
                "ZYMATICA_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    from zymatica_cli.plugins import PluginManager
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".zymatica" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from zymatica_cli.plugins import SUPPORTED_MANIFEST_VERSION
    ...
    if schema:
        from zymatica_cli.plugins import _CONFIG_SCHEMA_TYPES
