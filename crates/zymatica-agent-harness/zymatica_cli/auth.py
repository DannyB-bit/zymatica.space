...
is persisted in ~/.zymatica/auth.json with cross-process file locking.
...
from zymatica_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from zymatica_cli import __version__ as _ZYMATICA_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _ZYMATICA_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"hermes-cli/{_ZYMATICA_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://zymatica.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://zymatica.nousresearch.com/docs/guides/oauth-over-ssh"
...
def get_anthropic_key() -> str:
    ...
    ``~/.zymatica/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from zymatica_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from zymatica_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.zymatica/.env over os.environ so a deliberate key rotation
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from zymatica_cli.nous_account import (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ZYMATICA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.zymatica/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hermes_home() / "auth.json"
    # Seat belt: if pytest is running and ZYMATICA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ZYMATICA_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".zymatica" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ZYMATICA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ZYMATICA_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.zymatica/auth.json`` even when ZYMATICA_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped ZYMATICA_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".zymatica" / "auth.json"
            try:
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    if pconfig is None:
        from zymatica_cli.providers import get_provider
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from zymatica_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.zymatica/.env isn't silently
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.zymatica/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``ZYMATICA_PORTAL_BASE_URL`` /
    ...
    ``ZYMATICA_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("ZYMATICA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ZYMATICA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.zymatica/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("ZYMATICA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ZYMATICA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ZYMATICA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZYMATICA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZYMATICA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.zymatica/.env, and return it.
    ...
    print("       App name:     anything (e.g. zymatica)")
    ...
    save_env_value("ZYMATICA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ZYMATICA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ZYMATICA_SPOTIFY_CLIENT_ID to ~/.zymatica/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ZYMATICA_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.zymatica/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.zymatica/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.zymatica/auth.json)."""
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("ZYMATICA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZYMATICA_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.zymatica/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.zymatica/auth.json even when ZYMATICA_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.zymatica/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``ZYMATICA_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZYMATICA_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("ZYMATICA_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("ZYMATICA_CA_BUNDLE")
...
#
# File lives at ${ZYMATICA_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<hermes-root>/shared/nous_auth.json`` where ``<hermes-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.zymatica`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's ZYMATICA_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``ZYMATICA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`zymatica_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.zymatica/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``ZYMATICA_HOME`` deployments at
    ``<ZYMATICA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("ZYMATICA_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set ZYMATICA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set ZYMATICA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No ZYMATICA_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.zymatica/auth.json) called POST /api/oauth/token with Hermes's "
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # ZYMATICA_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("ZYMATICA_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ZYMATICA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ZYMATICA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from zymatica_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from zymatica_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ZYMATICA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from zymatica_cli.config import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    try:
        ...
        from zymatica_cli.model_search import model_search_text
    ...
    from zymatica_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from zymatica_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.zymatica/auth.json."""
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("ZYMATICA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.zymatica/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``ZYMATICA_PORTAL_BASE_URL`` set
