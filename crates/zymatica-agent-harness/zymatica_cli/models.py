...
from zymatica_cli import __version__ as _ZYMATICA_VERSION
from zymatica_cli.urllib_security import open_credentialed_url

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ZYMATICA_USER_AGENT = f"hermes-cli/{_ZYMATICA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from zymatica_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ZYMATICA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ZYMATICA_HOME/models_dev_cache.json directly (no network) so this
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from zymatica_cli.nous_account import get_nous_portal_account_info
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ZYMATICA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from zymatica_cli.model_catalog import get_default_model_from_cache
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    zymatica is tool-calling-first — every provider path assumes the model
...
# zymatica_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from zymatica_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — zymatica
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from zymatica_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    env_base = None
    try:
        from zymatica_cli.auth import _nous_inference_env_override
    ...
    creds_base = ""
    try:
        from zymatica_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from zymatica_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from zymatica_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.zymatica/.env``.
    ...
    """
    try:
        from zymatica_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from zymatica_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from zymatica_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``hermes model`` shows.
            from zymatica_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $ZYMATICA_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ZYMATICA_HOME/auth.json`` and external
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    """
    try:
        from zymatica_cli.copilot_auth import copilot_request_headers
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ZYMATICA_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ZYMATICA_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"zymatica/{_ZYMATICA_VERSION}"
