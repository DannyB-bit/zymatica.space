...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.zymatica/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.zymatica/skills/<name>/)[/]"
        )
    try:
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with zymatica but is not activated by default.\n"
    ...
    # metadata.zymatica.blueprint block, it is a runnable automation. Register it as
...
def do_list(source_filter: str = "all",
    ...
    ``skills.disabled`` list because ``-p`` swaps ``ZYMATICA_HOME`` at process
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from zymatica_cli/main.py."""
