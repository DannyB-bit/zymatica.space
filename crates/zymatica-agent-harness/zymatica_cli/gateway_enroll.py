...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.zymatica/auth.json``) — the same path ``hermes dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.zymatica/.env``. The per-gateway secret authenticates the WS upgrade;
...
def cmd_gateway_enroll(args) -> None:
    ...
    from zymatica_cli.config import is_managed, save_env_value
    ...
    #    save_env_value writes them to ~/.zymatica/.env (0600 dir) and never logs.
    to_write = {
