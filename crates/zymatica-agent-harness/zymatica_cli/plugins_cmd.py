...
Plugins are installed from Git repositories into ``~/.zymatica/plugins/``.
...
from zymatica_cli._subprocess_compat import noninteractive_git_env
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.zymatica/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from zymatica_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from zymatica_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from zymatica_cli.plugin_capabilities import parse_declared_capabilities
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from zymatica_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from zymatica_cli.plugin_capabilities import granted_capabilities
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    plugin directory under ``~/.zymatica/plugins``. Include package metadata here
    so ``hermes plugins list`` can show and enable them.
    """
    from zymatica_cli.plugins import ENTRY_POINTS_GROUP
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from zymatica_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.zymatica/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.zymatica/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.zymatica/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from zymatica_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from zymatica_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from zymatica_cli.plugin_packs import pack_command
