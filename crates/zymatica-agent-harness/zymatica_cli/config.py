...
Config files are stored in ~/.zymatica/ for easy access:
- ~/.zymatica/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zymatica/.env         - API keys and secrets
...
from zymatica_cli.route_identity import normalize_route_base_url
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.zymatica/config.yaml`` causes ``load_config()``
...
# * ``ZYMATICA_HOME`` / ``ZYMATICA_PROFILE`` / ``ZYMATICA_CONFIG`` /
#   ``ZYMATICA_ENV`` — Hermes runtime location flags. Writing these into
...
#
# IMPORTANT: ``ZYMATICA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ZYMATICA_LANGFUSE_PUBLIC_KEY,
# ZYMATICA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a ZYMATICA_* blanket: integration credentials (ZYMATICA_GEMINI_*,
    # ZYMATICA_LANGFUSE_*, ZYMATICA_SPOTIFY_*, ...) ARE allowed.
    "ZYMATICA_HOME", "ZYMATICA_PROFILE", "ZYMATICA_CONFIG", "ZYMATICA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(ZYMATICA_HOME, ZYMATICA_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.zymatica/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ZYMATICA_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    "ZYMATICA_LANGFUSE_ENV",
    "ZYMATICA_LANGFUSE_RELEASE",
    "ZYMATICA_LANGFUSE_SAMPLE_RATE",
    "ZYMATICA_LANGFUSE_MAX_CHARS",
    "ZYMATICA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "ZYMATICA_ACP_AUTH_METHOD",
    "ZYMATICA_ACP_AUTO_APPROVE",
    "ZYMATICA_COPILOT_ACP_COMMAND",
    "ZYMATICA_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from zymatica_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ZYMATICA_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZYMATICA_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the ZYMATICA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZYMATICA_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    the running interpreter, NOT of ``$ZYMATICA_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$ZYMATICA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ZYMATICA_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.zymatica``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ZYMATICA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.zymatica:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ZYMATICA_HOME``, a
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/zymatica`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ZYMATICA_HOME.
    try:
    ...
    #    host install whose shared $ZYMATICA_HOME was stamped by a co-located
    ...
    # detect Nix installs that don't set ZYMATICA_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zymatica_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ZYMATICA_HOME``, so that two installs sharing one data directory
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/zymatica:latest"
...
Hermes Agent runs as a published image (nousresearch/zymatica), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/zymatica:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate zymatica
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/zymatica:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/zymatica/tags
  • Your config and session history live under ``$ZYMATICA_HOME`` (``/opt/data``
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("ZYMATICA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(ZYMATICA_MANAGED={env_hint}).\n"
            "Edit services.zymatica.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZYMATICA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or ZYMATICA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZYMATICA_DEV") == "1":
        return None
    ...
    container_name = info.get("container_name", "zymatica")
...
# Re-export from zymatica_constants — canonical definition lives there.
from zymatica_constants import get_hermes_home, get_process_hermes_home  # noqa: F811,E402
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ZYMATICA_UID / ZYMATICA_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level ZYMATICA_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ZYMATICA_UID", "").strip()
    gid_str = os.environ.get("ZYMATICA_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``ZYMATICA_UID:ZYMATICA_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    The mode can be overridden via the ZYMATICA_HOME_MODE environment variable
    (e.g. ZYMATICA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZYMATICA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ZYMATICA_UID``/``ZYMATICA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZYMATICA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZYMATICA_CONTAINER") or os.environ.get("ZYMATICA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZYMATICA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZYMATICA_HOME, upgrading legacy empty templates.
...
_ZYMATICA_HOME_ENSURED: set = set()


def ensure_hermes_home():
    """Ensure ~/.zymatica directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ZYMATICA_HOME`` we only re-run
    ...
    if key in _ZYMATICA_HOME_ENSURED and home.is_dir():
        return
    ...
    # silently mkdir-ing the old ZYMATICA_HOME would resurrect an empty skeleton
    ...
    _ZYMATICA_HOME_ENSURED.add(key)
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"ZYMATICA_HOME {home} does not exist. "
...
from zymatica_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zymatica.config`` entries, then checks
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from zymatica_cli.config_migrations import (
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from zymatica_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from zymatica_cli.toolset_validation import validate_platform_toolsets
    ...
    # metadata.zymatica.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.zymatica/.env); "
...
from zymatica_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401
...
def _prompt_text(value: Any) -> str:
    ...
    """
    from zymatica_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from zymatica_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``ZYMATICA_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`hermes_cli.personality` (single owner).
    """
    from zymatica_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zymatica/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zymatica/config.yaml.
    ...
    (which change ``ZYMATICA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.zymatica/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zymatica/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zymatica/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zymatica/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ZYMATICA_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ZYMATICA_CUSTOM_{slug}_API_KEY" if slug else "ZYMATICA_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zymatica/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zymatica/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.zymatica/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.zymatica/.env`` over ``os.environ``.
...
def show_config():
    ...
    from zymatica_cli.auth import get_anthropic_key
    ...
    # Warn on stale ZYMATICA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ZYMATICA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ZYMATICA_MAX_ITERATIONS={_env_ghost} "
    ...
    try:
        from zymatica_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``ZYMATICA_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("ZYMATICA_MODEL", ""))
