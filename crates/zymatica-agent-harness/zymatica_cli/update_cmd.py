...
def _run_config_check_fresh() -> tuple:
    ...
    from zymatica_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from zymatica_cli.config import migrate_config
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from zymatica_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from zymatica_constants so this can't drift
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://zymatica.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.zymatica-update-staging"
    backup = f"{dst}.zymatica-update-old"
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``zymatica_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("ZYMATICA_ACTION_ID", "")
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/zymatica/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"zymatica-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for zymatica itself
        ...
        # GitHub ZIPs extract to zymatica-<branch>/
        extracted = os.path.join(tmp_dir, f"zymatica-{branch}")
        ...
        print(
            ...
            "reinstall from https://zymatica.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from zymatica_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from zymatica_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from zymatica_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/zymatica.git",
    "git@github.com:NousResearch/zymatica.git",
    "https://github.com/NousResearch/zymatica",
    "git@github.com:NousResearch/zymatica",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/zymatica.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/zymatica.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/zymatica.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/zymatica.git' to add later."
            )
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from zymatica_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zymatica_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ZYMATICA_HOME/bin/uv``, but on Termux the official
    ...
    """
    from zymatica_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from zymatica_constants import is_wsl
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.zymatica/logs/update.log`` only, never the terminal.
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from zymatica_cli.config import detect_install_method, recommended_update_command_for_method
    ...
    if method == "docker":
        ...
        from zymatica_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/zymatica).
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ZYMATICA_HOME under
    ...
    snapshot_id = None
    try:
        from zymatica_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from zymatica_constants import
        # get_hermes_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from zymatica_cli.config import get_hermes_home as _get_home
    ...
    try:
        from zymatica_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from zymatica_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_hermes_home so the user sees ~/.zymatica/...
    try:
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        # installer / desktop bootstrap stamps `.zymatica-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".zymatica-bootstrap-complete",
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from zymatica_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    managed-runtime descendants (the ``.zymatica-runtime`` interpreter child)
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.zymatica-runtime`` interpreter child alive and holding the install open
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from zymatica_cli import gateway_windows
        from zymatica_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    ``$ZYMATICA_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .zymatica-runtime child) holding the
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://zymatica.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_hermes_home added to zymatica_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in zymatica_constants or lazy_deps. The dependency install
        ...
        if not import_ok:
            ...
            print("    https://zymatica.nousresearch.com")
        ...
        # current — so copy it straight over ~/.zymatica/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ZYMATICA_HOME so
        # it is not affected by sync_skills()'s module-level ZYMATICA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZYMATICA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from zymatica_cli.profiles import backfill_profile_envs
        ...
        # the source files. A function-level ``from zymatica_cli.config import
        ...
        # it and warn loudly.
        try:
            from zymatica_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            ...
            try:
                from zymatica_cli.gateway import _get_restart_exit_wait_budget
