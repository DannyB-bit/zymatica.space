...
#: ZYMATICA_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    ...
    from zymatica_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$ZYMATICA_HOME/plugins/`` for currently installed user plugins.
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    #   3. Creates `~/.zymatica/logs/curator/` if missing (belt-and-suspenders
