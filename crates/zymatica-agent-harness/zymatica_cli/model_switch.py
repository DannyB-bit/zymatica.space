...
_NOUS_ZYMATICA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZYMATICA_NON_AGENTIC_RE.search(model_name))


def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _ZYMATICA_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zymatica_cli.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from zymatica_cli.route_identity import should_clear_context_pin
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from zymatica_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from zymatica_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from zymatica_cli.config import load_config
            from zymatica_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from zymatica_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from zymatica_cli.providers import nous_api_mode
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from zymatica_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from zymatica_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from zymatica_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from zymatica_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zymatica_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://zymatica.nousresearch.com/docs/api/model-catalog.json so
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (ZYMATICA_OVERLAYS) with proper auth store checking.
        ...
        from zymatica_cli.auth import is_runtime_provider_routable
        ...
        if not has_creds:
            try:
                from zymatica_cli.auth import _load_auth_store
    ...
    from zymatica_cli.providers import ZYMATICA_OVERLAYS
    from zymatica_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hermes provider ID.
    # ZYMATICA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hermes IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZYMATICA_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from zymatica_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or ZYMATICA_OVERLAYS (keeps /model in sync
    # with `hermes model`).
    try:
        from zymatica_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for grp in ep_groups.values():
            ...
            if _discovery_allowed:
                try:
                    from zymatica_cli.models import cached_fetch_api_models
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from zymatica_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    """
    from zymatica_cli.models import fetch_openrouter_models
