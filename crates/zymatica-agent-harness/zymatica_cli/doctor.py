...
from zymatica_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ZYMATICA_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.zymatica or ~/.zymatica/profiles/coder)

# Load environment variables from ~/.zymatica/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from zymatica_cli.colors import Colors, color
from zymatica_cli.models import _ZYMATICA_USER_AGENT
from zymatica_cli.vercel_auth import describe_vercel_auth
from zymatica_constants import OPENROUTER_MODELS_URL
...
from zymatica_constants import is_termux as _is_termux
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    ...
    from zymatica_cli.backup import _QUICK_STATE_FILES
...
def _format_db_size(db_path: Path) -> str:
    ...
    from zymatica_cli.backup import _format_size
...
def _report_database_journal_modes(
    ...
    home = hermes_home if hermes_home is not None else ZYMATICA_HOME
    try:
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.zymatica/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ZYMATICA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from zymatica_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from zymatica_cli.auth import get_xai_oauth_auth_status
...
from zymatica_cli.sizefmt import format_bytes as _human_bytes
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # ZYMATICA_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. ZYMATICA_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ZYMATICA_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ZYMATICA_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from zymatica_cli import __version__ as init_version
    except Exception:
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    """
    try:
        from zymatica_cli.gateway import (
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from zymatica_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ZYMATICA_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ZYMATICA_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ZYMATICA_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    os.environ.setdefault("ZYMATICA_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.zymatica/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    managed_scope_check()
    # Check ~/.zymatica/.env (primary location for user config)
    env_path = ZYMATICA_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.zymatica/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ZYMATICA_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from zymatica_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from zymatica_cli.config import is_provider_enabled
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from zymatica_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.zymatica/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from zymatica_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from zymatica_cli.config import check_config_version, migrate_config
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from zymatica_cli.config import atomic_config_write
        ...
        # Detect stale ZYMATICA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ZYMATICA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from zymatica_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ZYMATICA_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ZYMATICA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `hermes setup` run)",
                )
                if should_fix:
                    if remove_env_value("ZYMATICA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ZYMATICA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ZYMATICA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ZYMATICA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ZYMATICA_MAX_ITERATIONS in .env shadows config.yaml — "
        ...
        try:
            from zymatica_cli.config import load_env as _load_env_depr
            from zymatica_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        from zymatica_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'zymatica[vercel]')",
                "Install the Vercel optional dependency: pip install 'zymatica[vercel]'",
                issues,
            )
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ZYMATICA_HOME mirror
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _ZYMATICA_USER_AGENT,
            }
    ...
    _section("Skills Hub")
    hub_dir = ZYMATICA_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        from zymatica_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ZYMATICA_HOME / "config.yaml"
    ...
    try:
        from zymatica_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    try:
        from zymatica_cli.doctor_live import maybe_run_live_checks
