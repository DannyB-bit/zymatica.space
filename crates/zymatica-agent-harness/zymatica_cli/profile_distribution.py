...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "zymatica", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zymatica_requires:
            out["hermes_requires"] = self.zymatica_requires
        if self.author:
...
def plan_install(
    ...
    from zymatica_cli import __version__ as hermes_version
    ...
    check_hermes_requires(manifest.zymatica_requires, hermes_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.zymatica).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
