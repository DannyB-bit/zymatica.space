...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ZYMATICA_HOME?"""
...
def _resolved_home() -> str:
    hermes_home = os.environ.get("ZYMATICA_HOME", "").strip()
    if hermes_home:
        return hermes_home
    return os.path.join(os.path.expanduser("~"), ".zymatica")
...
def container_mode_may_be_active() -> bool:
    ...
    default_home = os.path.join(os.path.expanduser("~"), ".zymatica")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ZYMATICA_HOME.
    """
...
def print_fast_version_info() -> None:
    from zymatica_cli import __release_date__, __version__
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    subcommand + the ZYMATICA_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("ZYMATICA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
