...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from zymatica_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local ZYMATICA_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from zymatica_constants import set_hermes_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from zymatica_constants import reset_hermes_home_override
...
def _resolve_discovery_timeout(
    ...
    try:
        from zymatica_cli.config import load_config, DEFAULT_CONFIG
