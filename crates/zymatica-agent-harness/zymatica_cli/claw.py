...
from zymatica_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _cmd_migrate(args):
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.zymatica/. "
    ...
    if not no_backup:
        try:
            from zymatica_cli.backup import create_pre_migration_backup, _format_size
