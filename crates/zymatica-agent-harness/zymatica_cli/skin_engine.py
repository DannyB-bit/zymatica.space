...
file. Skins are defined as YAML files in ~/.zymatica/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.zymatica/skins/ therefore
...
.. code-block:: python
    ...
    from zymatica_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.zymatica/skins/
...
Drop a YAML file in ``~/.zymatica/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces ZYMATICA_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces ZYMATICA_CADUCEUS)
