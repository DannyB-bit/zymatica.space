...
verbatim from zymatica_cli/config.py. Must not import from zymatica_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The ZYMATICA_ENVIRONMENT_HINT
        ...
        # ``ZYMATICA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             ZYMATICA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ZYMATICA_HOME/home when it exists (old strict
        ...
        #  "/home/user/.zymatica/cache/documents:/output"]
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zymatica/checkpoints/`` size (MB).  When
    ...
    # AGENTS.md, CLAUDE.md, .zymatica.md, or .cursorrules before Hermes applies
    ...
    "display": {
        ...
        # REPL and `--tui` (or ZYMATICA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hermes --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hermes -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZYMATICA_TUI_RESUME=<id> always wins.
        ...
        # can set `path`/`paths` to ZYMATICA_HOME-relative YAML files/directories
    ...
    "dashboard": {
        ...
        # ``ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ZYMATICA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ZYMATICA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ZYMATICA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ZYMATICA_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zymatica/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zymatica/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.zymatica/cache/delegation/
    ...
    # always goes to ~/.zymatica/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZYMATICA_SKILL_DIR} and ${ZYMATICA_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.zymatica/skills/ into
        # ~/.zymatica/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ZYMATICA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.zymatica/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZYMATICA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .zymatica config) always require human
    ...
    "cron": {
        ...
        # $ZYMATICA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ZYMATICA_CRON_MAX_PARALLEL env var.
        ...
        # ZYMATICA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    "code_execution": {
        ...
        #     zymatica's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.zymatica/logs/.
    ...
    # update model picker lists without shipping a zymatica release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://zymatica.nousresearch.com/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # internal ZYMATICA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.zymatica/sessions/sessions.json entirely.
        ...
        # the ZYMATICA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.zymatica.gateway` that defenses 1-2 don't
        ...
        # ``ZYMATICA_GATEWAY_MAX_STARTS`` / ``ZYMATICA_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.zymatica/.env,
        ...
        # user. Bridged to ZYMATICA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # cache (~/.zymatica/cache/{documents,images,audio,video,screenshots})
        ...
        # to ZYMATICA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ZYMATICA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ZYMATICA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.zymatica/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.zymatica/sessions/session_{sid}.json`` on every turn
        ...
        # ~/.zymatica/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ZYMATICA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ZYMATICA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    ...
    "updates": {
        ...
        #     DBs) into <ZYMATICA_HOME>/state-snapshots/ before the update.
        ...
        #     of ZYMATICA_HOME into <ZYMATICA_HOME>/backups/, restorable with
    ...
    "lsp": {
        ...
        #              ``<ZYMATICA_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.zymatica/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.zymatica/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.zymatica/cache/. If a later startup cannot reach
            ...
            # ~/.zymatica/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.zymatica/.env (or the shell)
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.zymatica/bin/ on
    ...
    "desktop": {
        ...
        # Bridged to the ZYMATICA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the ZYMATICA_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZYMATICA_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zymatica' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # ZYMATICA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ZYMATICA_TOOL_PROGRESS_MODE for backward
    ...
    "ZYMATICA_PREFILL_MESSAGES_FILE": {
    ...
    "ZYMATICA_EPHEMERAL_SYSTEM_PROMPT": {
