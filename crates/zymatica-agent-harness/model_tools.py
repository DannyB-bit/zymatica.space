...
def _is_dispatcher_owned_worker() -> bool:
    """False when ZYMATICA_KANBAN_* is present but this execution does not own it
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("ZYMATICA_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            os.environ.get("ZYMATICA_KANBAN_TASK")
            and not _is_delegated_child_context()
            and _is_dispatcher_owned_worker()
            and "kanban" not in effective_enabled_toolsets
        ):
            # Dispatcher-spawned workers are scoped by ZYMATICA_KANBAN_TASK and
        ...
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("hermes-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (hermes-*) include _ZYMATICA_CORE_TOOLS, and
    ...
    # window). Core Hermes tools (toolsets._ZYMATICA_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from zymatica_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to zymatica's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from zymatica_cli.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from zymatica_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from zymatica_cli.plugins import resolve_pre_tool_block
        ...
        try:
            ...
            if skip_tool_execution_middleware:
                ...
                from zymatica_cli.middleware import run_tool_execution_middleware
