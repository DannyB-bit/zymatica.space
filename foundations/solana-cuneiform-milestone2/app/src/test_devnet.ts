...
async function main() {
  ...
  try {
    state = await client.fetchProgramState();
    if (!state) throw new Error("Program state not found");

    if (state.feeLamports !== BigInt(100_000)) {
      console.log(`🔄  Updating devnet program state fee from ${state.feeLamports} to 100,000 lamports...`);
      await client.updateProgramState(deployer, undefined, BigInt(100_000));
      const updatedState = await client.fetchProgramState();
      if (updatedState) {
        state.feeLamports = updatedState.feeLamports;
      }
    }
    ...
    if (state.feeLamports === BigInt(100_000)) {
      pass("Protocol fee = 100,000 lamports");
      passed++;
    } else {
  ...
  try {
    ...
    if (feePaid >= Number(state.feeLamports)) {
