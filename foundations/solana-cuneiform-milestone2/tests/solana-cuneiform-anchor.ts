...
async function runTests() {
  ...
  if (!recordPda) throw new Error("PDA Derivation failed");
  ...
  client.connection.getAccountInfo = async (pubkey: PublicKey) => {
  ...
  if (!record) throw new Error("Failed to fetch record");
  ...
  // Assertions
  if (!record.authority.equals(authority.publicKey)) throw new Error("Authority mismatch");
  ...
  const rewardDetails = payGateway.generatePaymentRequest(
    authority.publicKey, // recipient node
  ...
  if (!rewardDetails.reference) throw new Error("Solana Pay request failed");
