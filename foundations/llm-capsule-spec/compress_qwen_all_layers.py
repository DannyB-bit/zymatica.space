...
def pack_chirps(payload):
    needed_data = math.ceil(len(payload) / DATA_PER_PKT)
    needed_data = max(needed_data, 1)
    capacity = needed_data * DATA_PER_PKT
    padded=(payload+(WATERMARK*(capacity//len(WATERMARK)+1)))[:capacity]
    chunks=[padded[i*DATA_PER_PKT:(i+1)*DATA_PER_PKT] for i in range(needed_data)]
    
    total_pkts = needed_data + 1
    data=[bytes([0xBB,i,total_pkts])+c for i,c in enumerate(chunks)]
    parity=bytearray(DATA_PER_PKT)
    for c in chunks:
        for j in range(DATA_PER_PKT): parity[j]^=c[j]
    return data+[bytes([0xBB,needed_data,total_pkts])+bytes(parity)]
...
def main():
    ...
    print("="*72, flush=True)
    print("  FULL QWEN 3.5 0.8B → 5 × 255-BYTE LoRa CHIRPS", flush=True)
    ...
    WEIGHT_BUDGET = float('inf')
    print(f"  Weight budget: UNLIMITED", flush=True)
    ...
    if not fits:
        print(f"\n  [NOTE] Payload exceeds 5 chirps. Keeping all layers for full-model proof.", flush=True)

    # ── Pack into chirps ───────────────────────────────────────────────────
    print(f"\n[CHIRPS] Packing into {PKT_SIZE}-byte LoRa chirps...",
          flush=True)
    ...
    for i, chirp in enumerate(chirps):
        ...
        sha.update(chirp)
        tag="XOR-FEC" if i==(len(chirps)-1) else f"DATA-{i}"
    ...
    print(f"", flush=True)
    print(f"  LoRa Chirps:           {len(chirps)} × {PKT_SIZE} bytes  =  {len(chirps)*PKT_SIZE} bytes total", flush=True)
    print(f"  Compression ratio:     {st_size//max(1, len(chirps)*PKT_SIZE):>15,}× vs raw model", flush=True)
