...
=====================================================================
Full Qwen 3.5 0.8B → 9 × 255-byte LoRa Chirps
...
def encode_layer(lid: int, tensor: torch.Tensor):
    ...
    # L6: gradient atom from top-K singular values (skipping L5 completely)
    ...
    # Energy in top-K vs Frobenius norm to calculate alignment %
    frob_sq = float(torch.sum(tensor.float() ** 2))
    energy_K = float(torch.sum(S_k ** 2))
    align = energy_K / max(frob_sq, 1e-9)
...
def main():
    ...
    print("="*72, flush=True)
    print("  FULL QWEN 3.5 0.8B → 9 × 255-BYTE LoRa CHIRPS", flush=True)
    ...
    manifest={
        ...
        "method":"L7:QualiaSeed+L6:GradAtom+L1:LU4QA+ZLIB",
        "watermark":"ip zymatica.space",
