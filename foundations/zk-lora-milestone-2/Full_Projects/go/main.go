...
// ============================================================================
//go:embed zk_lora_wasm_verifier.wasm
var wasmBytes []byte
...
func NewZKProver() *ZKProver {
	...
	// Seed the PRNG in WASM with secure random bytes from Go
	...
	seedPrng := mod.ExportedFunction("seed_prng")

	results, err := alloc.Call(ctx, uint64(len(seedBytes)))
	if err != nil {
		log.Fatalf("failed to allocate memory in WASM: %v", err)
	}
	seedPtr := results[0]

	if !mod.Memory().Write(uint32(seedPtr), seedBytes) {
		log.Fatalf("failed to write seed to WASM memory")
	}

	_, err = seedPrng.Call(ctx, seedPtr, uint64(len(seedBytes)))
	if err != nil {
		log.Fatalf("failed to seed PRNG in WASM: %v", err)
	}

	_, err = dealloc.Call(ctx, seedPtr, uint64(len(seedBytes)))
	if err != nil {
		log.Fatalf("failed to deallocate seed memory in WASM: %v", err)
	}
...
func (p *ZKProver) Close() {
	p.mod.Close(p.ctx)
	p.runtime.Close(p.ctx)
}

func (p *ZKProver) GenerateProof(privateKeyHex, publicKeyHash string) ZKProof {
	alloc := p.inst.ExportedFunction("alloc")
	dealloc := p.inst.ExportedFunction("dealloc")
	genProof := p.inst.ExportedFunction("generate_zk_proof_wasm")

	// Helper to write string to WASM
	writeString := func(str string) (uint64, uint64) {
		bytes := []byte(str)
		res, err := alloc.Call(p.ctx, uint64(len(bytes)))
		if err != nil {
			log.Fatalf("failed to allocate memory: %v", err)
		}
		ptr := res[0]
		if !p.inst.Memory().Write(uint32(ptr), bytes) {
			log.Fatalf("failed to write string to WASM memory")
		}
		return ptr, uint64(len(bytes))
	}

	privPtr, privLen := writeString(privateKeyHex)
	pubPtr, pubLen := writeString(publicKeyHash)

	maxLen := uint64(4096)
	resAlloc, err := alloc.Call(p.ctx, maxLen)
	if err != nil {
		log.Fatalf("failed to allocate output buffer: %v", err)
	}
	outPtr := resAlloc[0]

	resGen, err := genProof.Call(p.ctx, privPtr, privLen, pubPtr, pubLen, outPtr, maxLen)
	if err != nil {
		log.Fatalf("failed to call generate_zk_proof_wasm: %v", err)
	}
	outLen := resGen[0]

	if outLen == 0 {
		log.Fatalf("failed to generate proof in WASM")
	}

	// Read output JSON from WASM memory
	jsonBytes, ok := p.inst.Memory().Read(uint32(outPtr), uint32(outLen))
	if !ok {
		log.Fatalf("failed to read JSON from WASM memory")
	}
	...
	// Deallocate
	_, _ = dealloc.Call(p.ctx, privPtr, privLen)
	_, _ = dealloc.Call(p.ctx, pubPtr, pubLen)
	_, _ = dealloc.Call(p.ctx, outPtr, maxLen)

	return ZKProof{
		ProofA:       parsed.ProofA,
		ProofB:       parsed.ProofB,
		ProofC:       parsed.ProofC,
		ProofHash:    parsed.ProofHash,
		PublicInput:  publicKeyHash,
		CeremonyHash: "0000000000000000",
		Protocol:     "groth16",
		Curve:        "bn254",
...
func (p *ZKProver) VerifyProof(proof ZKProof, publicKeyHash string) bool {
	...
	alloc := p.inst.ExportedFunction("alloc")
	dealloc := p.inst.ExportedFunction("dealloc")
	verifyProof := p.inst.ExportedFunction("verify_zk_proof_raw")
	...
	aPtr, aLen := writeString(proof.ProofA)
	bPtr, bLen := writeString(proof.ProofB)
	cPtr, cLen := writeString(proof.ProofC)
	pubPtr, pubLen := writeString(proof.PublicInput)

	if aPtr == 0 || bPtr == 0 || cPtr == 0 || pubPtr == 0 {
		return false
	}

	resVerify, err := verifyProof.Call(p.ctx, aPtr, aLen, bPtr, bLen, cPtr, cLen, pubPtr, pubLen)
	if err != nil {
		return false
	}
	isValid := resVerify[0]
	...
	// Deallocate
	_, _ = dealloc.Call(p.ctx, aPtr, aLen)
	_, _ = dealloc.Call(p.ctx, bPtr, bLen)
	_, _ = dealloc.Call(p.ctx, cPtr, cLen)
	_, _ = dealloc.Call(p.ctx, pubPtr, pubLen)

	return isValid == 1
}

func MimcHash(privateKeyHex string) string {
	...
	current := new(big.Int)
	for i := len(privBytes) - 1; i >= 0; i-- {
		current.Lsh(current, 8)
		current.Add(current, big.NewInt(int64(privBytes[i])))
	}
	current.Mod(current, prime)

	for i := 0; i < 10; i++ {
		c := big.NewInt(int64(i + 123456789))
		tmp := new(big.Int).Add(current, c)
		tmp.Mod(tmp, prime)

		tmpSq := new(big.Int).Mul(tmp, tmp)
		tmpSq.Mod(tmpSq, prime)

		current.Mul(tmpSq, tmp)
		current.Mod(current, prime)
	}

	// Encode as 32-byte little-endian hex string
	outBytes := make([]byte, 32)
	temp := new(big.Int).Set(current)
	for i := 0; i < 32; i++ {
		outBytes[i] = byte(temp.Uint64() & 0xff)
		temp.Rsh(temp, 8)
	}
