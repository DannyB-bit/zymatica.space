...
def run_proof():
    ...
    if os.path.exists(__file__):
        ...
        code_bytes = b"python_proof_code"
    v_hash = hashlib.sha256(code_bytes).hexdigest()
    r_sig = hashlib.sha256(f"{tx_id}:{v_hash}".encode()).hexdigest()
    print(f"      Validator Binary Hash: {v_hash[:32]}...")
    print(f"      Signed Routing Receipt: {r_sig[:32]}...")
